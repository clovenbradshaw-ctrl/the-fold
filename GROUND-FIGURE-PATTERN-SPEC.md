...
the keyless memory, in three tiers — holds nodes that are a state, a payload and their synapses, and offers
exactly four ways in — `recall(cue)`, `after(node)`, `before(node)`,
`bind(a, b)` — and no `get`. A node is one of three tiers (THE_HOLOGRAPH /
THE_SHADOW / THE_ECHO): the *holograph* keeps the full tokens AND the address
— both sides, the record merged, re-expandable to the ground, the only tier
the mouth reads; the *shadow* keeps only its state and the address — no words,
recall-only, re-expands only through the record; the *echo* keeps a coarse
state and the address — the minimum, never read back into full EOT. A *recall* activates every node by overlap with
