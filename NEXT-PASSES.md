...
The spec numbers its passes 32–42 "continuing from Pass 31"; this file already carries a Pass 32 (the thinking-depth slider, LANDED). Both stand; the spec's are referred to as **GFP Pass N** here. **GFP Pass 32 — LANDED 2026-09-07 (P169):** organs seated with cube-confirmed cells, seven tests kept, the §0 table reproduced to the probe. **GFP Pass 33 — LANDED 2026-09-08 (field-of-record.js / field-store.js / eval/field-of-record.mjs, wired into app.js's boot, sync, the reader loop and the record's own append):** THE SHADOW (the keyless field — relative.js's Field, by its 2026-09-11 name) derived from the record and the sources, rows without positions, a shuffled store rebuilds the same shadow, appends never rewrite; the §0 table reproduced over six kinds of real material (`eval/field-of-record.mjs`). **GFP Pass 35 — LANDED 2026-09-11 (recallForTurn in field-of-record.js; the `fieldRecall` seat in holon.js runPart, threaded through runHolonicTask; `fieldRecallNow()` + the `witnessed` reflex act in app.js; eval/field-witness.mjs):** the shadow offered beside lexical retrieval — a recalled passage the turn's pool holds but lexical missed is promoted (marked `retrievedVia: "relative"`, capped at FIELD_OFFER_MAX); one beyond the pool is recorded and never fed to the mouth (GFP P3 — a shade cannot be its own light source). Measured on pg2600 (400 passages, 120 fragment probes): the shadow bound 120/120 against lexical's 107/120 — gained 13, lost 0, 0 false figures — and the spec's own shuffled-record null resolved against spreading: steps=1 == steps=0 == shuffled (120/120 every arm), so **the turn seat runs steps=0, spreading off for this use**. Next in the spec's order: Pass 34 (the pattern act at reopen), then Pass 36 (cost at scale). **THE IMPRESSION — LANDED 2026-09-11** (relative.js textless nodes; `THE_IMPRESSION`, `impressionOf`, and the `impression: true` admit path in field-of-record.js; tested): the shadow's thin form, the minimum we remember — a node with a state and a pointer, NO words; it recalls ("have I met this"), names its origin, and can never be re-expanded or re-anchored (structurally unreadable, the sealed form to share — Pass 39). Measured at admission: 5/5 recall parity with the full shadow on every probe, mixed-store rebuild intact. The size knob is the state's resolution (SDR_BITS) — **Pass 36 measures it, never picked by hand**; the witness-confirmed paraphrase orbit (the semantic "embedding style") is Pass 37's build.
