...
The spec numbers its passes 32–42 "continuing from Pass 31"; this file already carries a Pass 32 (the thinking-depth slider, LANDED). Both stand; the spec's are referred to as **GFP Pass N** here. **GFP Pass 32 — LANDED 2026-09-07 (P169):** organs seated with cube-confirmed cells, seven tests kept, the §0 table reproduced to the probe. **GFP Pass 33 — LANDED 2026-09-08 (field-of-record.js / field-store.js / eval/field-of-record.mjs, wired into app.js's boot, sync, the reader loop and the record's own append):** the field derived from the record and the sources, rows without positions, a shuffled store rebuilds the same field, appends never rewrite; the §0 table reproduced over six kinds of real material (`eval/field-of-record.mjs`). **GFP Pass 35 — LANDED 2026-09-11 (recallForTurn in field-of-record.js; the `fieldRecall` seat in holon.js runPart, threaded through runHolonicTask; `fieldRecallNow()` + the `witnessed` reflex act in app.js; eval/field-witness.mjs):** the figure offered beside lexical retrieval — a recalled passage the turn's pool holds but lexical missed is promoted (marked `retrievedVia: "relative"`, capped at FIELD_OFFER_MAX); one beyond the pool is recorded and never fed to the mouth (GFP P3). Measured on pg2600 (400 passages, 120 fragment probes): the field bound 120/120 against lexical's 107/120 — gained 13, lost 0, 0 false figures — and the spec's own shuffled-record null resolved against spreading: steps=1 == steps=0 == shuffled (120/120 every arm), so **the turn seat runs steps=0, spreading off for this use**. Next in the spec's order: Pass 34 (the pattern act at reopen), then Pass 36 (cost at scale).
