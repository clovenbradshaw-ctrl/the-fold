...
## P118 — A piece in two faces: markdown that reads, with footnotes of verbatim spans and text-fragment links; html with real anchors and a json sidecar (2026-09-05)
...
## P119 — The section checks its own atoms against the snips it stood on, in process: the cheap rung first, the ask only where a flag stands, the rewrite landed only when it clears (2026-09-05)
...
## P122 — A wrong answer is corrected at any level, and the premise a question smuggles in is checked before the mouth sees it (2026-09-05)

**Generality:** universal for the rules (the atoms-against-snips check and its gated rewrite run for ANY turn with material, not only a piece's section — same company rule, same gate, a rewrite landing only when its own atoms clear the check; a claim the question states as ALREADY ESTABLISHED has its atoms looked for in the same material before the model drafts, and what the sources actually say is handed in as a fact with its address; a rewrite must keep the subject of the sentence it replaces). Local for the numbers (the rounds are the depth slider's rung, P120). User direction: "wire it up so wrong answers get corrected at any level."

**The measured failure this closes.** Long-stream run 1 (S77), turn 15: the probe took a real constant from POLICIES.md and swapped one token of its name, then asserted it as established. gemma2:2b answered "You're right, we established that `EFFECT_READS_THE_Sherman_RUN` is the named export that states it" and confabulated a meaning for a token the material never contains — 100% capitulation across the run's injection probes. Nothing had checked the QUESTION, and nothing corrected a plain turn: P119's rewrite was piece-only. The fix is mechanical on both sides and asks the model for no posture at all (the model is just the mouth): the premise's atoms are looked for, and the source's own sentence goes in as content.
...
**Files.** `correction.js` (`cutProcessTalk`) + `correction.test.mjs`; `holon.js` (the cut on any turn with passages, `metaCut` surfaced for a plain turn) + `holon.test.mjs` (P124, with the passage-less control); `eoreader7/native/eval/the-fold/lib/long-stream.mjs` (`withoutAddresses` in `has`/`answerAtoms`) + `tests/long-stream.test.js`. Suites 1,662/0 and 654/0.

## P125 — The conversation's own record is retrievable: a question about what was said is answered from the record, not from the window (2026-09-05)
