...
**Generality:** universal for the three rules (a walk-based driver refuses absent fixtures instead of narrowing; a driver whose opt-in organ no longer reaches the reader refuses instead of tabulating identical arms; a print-only results doc is enforced by moving its computation into `lib/` and reading it from `node --test`) — structural, no tuned number, each built to fail one way and pass the other before touching real material (eoreader7 **S65** carries the mechanism, the nine classifications and the file list); every count is specimen-scoped and dated.

**Where P94 stopped.** Three of thirteen wipe-exposed drivers audited, the fixture decision left open, nine drivers named and unrun. This pass took the decision, ran the nine with no cap, and made the four transcription docs enforceable on the `lib/coref-agreement.mjs` + `mhc-order5-precision.test.js` template — one `lib/` function per driver, one test that reads it every run.

**The decision: refuse.** A null over a silently narrowed pool was the worst of the three outcomes P94 named, and committing the 86 untracked faces would reverse the 09-04 rule for a doc that is a dated record of one run, not a claim about the repo. So `cited-source-null` and `ordered-read-reach` now exit 2 with a typed `fixture_absent` gap naming the walk's count, the absent count and a sample, and their docs say "reproducible only where the walk's fixtures exist". On this checkout: 106 named, 20 present, 86 absent, both refuse. The test pins the mechanism on synthetic walks and discloses the real state — the same posture as the order-5 refusals: assert the rule, log the specimen.

**The nine, classified — the short table; S65 has the numbers.** Reproduces: `full-circuit` (exactly; its heading says nine walls over ten rows), `mechanical-reasoning` (JSON byte-identical), `pruning-timeline` (every number; a stale self-path string in the JSON regenerated), `reasoning-e2e-no-llm` (every verdict — while its display printed `undefined —undefined→ undefined` for every edge: S64's unmigrated SVO reads in a second driver, now migrated at one seam). Drifted with the finding intact: `hyperlexicon-door-probe` (two new notes are the article's Russian title `Война —и→ мир`, a Cyrillic "and" reaching the ledger as a label the English hand list and POS prior cannot see), `subject-wall` (one to six edges per arm, not isolated to one organ; also its book path was pinned to another machine's `/home/user`, which no other checkout could reproduce), `rashomon-contrast` (the pool moved and the single shared claim is gone — the cast furniture wall this doc itself asked for, P93, and S64's surface rules are the candidates, not isolated; 0.0% both arms stands; its JSON is gitignored and its slot-level table was never computed by its driver). Unreproducible by construction: `object-boundary` (the `boundedObjects` opt-in its verdict kept was removed by this repo's own P80, `2214e1a`, the day after the doc; the re-run printed three byte-identical arms and "moved by the cut: 53" — address collisions — P41 in a third driver), `vendored-prior-eval` (its corpus is research-use text never committed). And `asserted-eval`, which P94 could not run: here the submodule is initialized, and it reproduces to the second.

**The law, extended by one clause.** P94: a number a policy stands on is either pinned by a test that reads it, or the entry says it is not. P95 adds the driver's side: **a driver that cannot measure what its doc measured says so, typed, and exits non-zero — it never prints a table about the material when the fact is about the checkout, the reader, or the invocation.** Three shapes of that fact were found in nine drivers: a fixture the checkout lacks (`fixture_absent`: the walk faces, the Dracula path, the vendored corpus), an organ the reader no longer reaches (`organ_unreachable`: `boundedObjects`), and an invocation that cannot supply what the arms need (`control_absent`: the MHC battery run on one material, which used to read "none readable"). All three are P41 — the instrument's own state wearing the material's face — and each now has a test that would fail if the refusal stopped firing.

**What the enforcement pins, and what it only discloses.** Built material is pinned exactly: `full-circuit`'s relay and `reasoning-e2e`'s three passages are constructions, so their numbers are the construction's own. A specimen is pinned by its rule and disclosed by its counts: the door probe asserts that the prior admits no hand-listed closed-class label and that the gate turns away nothing the reader had not refused, and logs 36/30/18 and 16/12/0 beside them; the object-boundary test asserts the refusal fires on a reader that ignores the opt-in and not on one that applies it, and logs which state the production reader is in. Asserting a specimen's count freezes a reader that must be allowed to move; asserting a known gap freezes it as a target; both were declined, as `mhc-order5-precision.test.js` declined them.

**The runner.** `eoreader7/native/eval/the-fold/audit-results.sh` carries the driver→artifact→enforcement map and reports `reproduces` / `DRIFTED` / `refused, typed: <gap>` / `print-only; enforced by: <test or none>` — never a vacuous zero. Two docs it names as enforced by nothing: `admission-gate` and `rashomon-contrast`.

**This repo.** No code moved. The-fold's `hypergraph.js` and `cast.js` already re-export the engine's organs, so the tests that read the engine's adapters read what a live turn reads; `verification.js` and `grid.js` are the two cross-repo imports the reasoning test makes, with a typed skip when the-fold is not beside the engine. CLAUDE.md pointer; `NEXT-PASSES.md` Pass 13 closed, Pass 14 opened with what this pass handed forward.

**Suites, failure names diffed.** the-fold 1,581 tests: 1,578 pass / 3 fail before and after — the same three names (the two EOReader-contract tests and the SELF/MATERIAL block test), none of which this pass touched. eoreader7 native: 593 → 619 tests, 592 → 618 pass, 0 fail throughout, the one `# TODO` unchanged.

## P96 — Zero-call hygiene before the circuit: a caller's render is tested against the organ's real return, and the reader's frame is derived, never restated (2026-09-05)

**Generality:** universal for the two rules (a render of an organ's return value is pinned against that organ's real return shape, never against the shape the caller remembers; a reader's configuration frame is derived from the options object the reader was built with, so a lever cannot be handed to the reader without reaching the record) — structural, no tuned number, each with a test that fails one way and passes the other; every count is specimen-scoped and dated. eoreader7 **S66** carries the driver side.

**Where this sits.** Pass 13 landed (P95/S65) and Pass 14 named five things the finished audit handed forward. The program that follows is one thing: make the existing machinery one functional Fold — every part of the product bar (an addressed sentence, a disclosed single witness, a derived fact with its weakest premise, a typed open contest, a claim set identical across a model swap) exists engine-side and is eval-only, unwired, or in-memory-only in the app. The plan of record names Passes 15–25 and the terminal live assay that defines "there". This is Pass 15: the zero-call floor, taken first because a number measured on a red suite through a render that throws is the instrument's state wearing the material's face (P90, P41).

**Three failures, three names, three causes — none a regression.** the-fold's suite had carried 3 failures since before P94, and P94/P95 each recorded them as "the same three names, untouched". Read rather than carried: (1) `eoreader-contract.test.mjs:26` was a REAL drift — `app.js` imports `/engine-v7/adapters/text/morphology.js` (commit `94be94e`, the morphology prior feeding the witness's company wall) and the contract's `browserNativeModules` did not list it; the contract's own migrationLaw says the two move in one commit, and the test was the ratchet doing its job with nobody reading it. Fixed by declaring the import. (2) `eoreader-contract.test.mjs:41` was the HARNESS: `importedNames` built `import\s*\{([\s\S]*?)\}\s*from "<module>"`, a lazy class that opened at the FILE's first `import {` and ran to the first `} from "<module>"`, so `explore-worker.mjs` — whose declared module is not its first import — returned every earlier import's names glued together. Read for weeks as a contract drift; it was a regex. Anchored on one statement's own braces (`[^}]*`). (3) `reflex.test.mjs:139` asserted `buildSourceBlock(...)` starts with `MATERIAL`; P80 retired that banner on purpose ("provenance here, duty there, each said once") and the test was never migrated — a check asserting a rule the code had retired, P94's class one register over. It now asserts the two planes differ by what they say. Measured: 1,581 / 1,578 / 3 → **1,588 / 1,588 / 0**, names diffed.

**The `/corroborate` door threw after landing the first live dispute.** `corroborateLedger` returns `standings` as a KEYED object — `{settled, contested, disconfirmed, thin}`, lists of note ids from the walk's own `askValue` verdicts — and, since P88, `contradicted` entries that are LANDED disputes (each carrying `disputeId`, `landed`, `refused`). `app.js` called `report.standings.filter(...)` and printed "contradicted (reported, never landed)". So the one app-reachable path that lands CON·Figure·CONTESTED wrote the ledger back and then crashed its own render, and its usage text still promised the two-source gate P84 removed. The render now lives in `corroboration-report.js` (pure) and `corroboration-report.test.mjs` pins it against the organ's real return — a landed dispute and a refused one counted apart, settled notes read from `standings.settled`, the old list-shaped assumption refused without throwing. **The rule:** a caller's render of an organ's return is a claim about that organ and is tested against the organ, not against the caller's memory of it. An organ whose tests never reach its caller's render passes forever while the caller crashes.

**The reader's frame, derived.** P90 asks for the configuration on the record. The app had one (`readerFrame`, landed as DEF·Ground with a recipe id on every witness) — written BY HAND as a second object beside the options the reader was built with, and the two had drifted: `attestedVerbs`, `phrasalPredicates` (DR5), the vocabulary POS gate's on/off and the cast organ that resolves ends were all live and none was in the frame, so two readings that differed in a lever shared a recipe. A restatement drifts (P22/P24/P39's class); a derivation cannot. The options object is now named (`RELATION_READER_OPTIONS`) and `reader-frame.js::readerFrame` computes the frame FROM it: every function-valued option is an organ named by itself, every other value a lever carried as its value, every null an omission; the loaded state of the received priors and the identity organ (`makeCastResolver`, with `noteIdentity` named as the deliberate omission it still is) are passed in as the two facts the options object cannot carry. `reader-frame.test.mjs` pins the property the whole thing exists for: an option added to the reader changes the frame, and a lever flipped is a different frame — hence a different recipe — with insertion order irrelevant. Recipe ids minted before this entry are not comparable to those after; the frame is on the record either way.

**Pass 14's items 4, 5 and the flag half of 2 — closed** (S66 has the per-driver reading). The SVO grep hits were read, not counted: drivers that map claims to `subject/verb/object` at their own seam (the cited-source pair, rashomon-contrast, copresence, the MHC battery's `sae`) are legitimate; `complicated-reading` reads the adapter's triples, which still carry `subject`; three read the wiped names off organ edges or claim rows and were migrated at one seam each — `mine-1-official-graph` (every node it would write was `undefined`), `try-grid-on-transcript` (`e.subject.toLowerCase()` threw on every edge), `material-dialogue-stress` (every crown candidate skipped as "empty subject"). `asserted-eval` prints its wall-clock to stderr and its tracked doc diffs to zero. The `BOUNDED` flag came out of `lib/borodino-ledger.mjs`. Items 1 (a wall for a label the prior cannot classify) and 3 (the rashomon slot-grain table) are carried to Pass 25, after the circuit.

**What follows (the plan of record, stated once here so the record does not depend on a file outside the repo).** Pass 16: the product assay harness on `gate-proof.mjs`'s shape, its 0-call arm pinned by a test, the AnswerRecord shape, the constitutions pinned by content identity. 17: one durable reading record (OPFS, append-only, replay = byte-identical fold; a mutated event must not fold identical). 18: read when material arrives, resumable, prefix-invariant. 19: claims before prose, and the model-swap claim diff. 20: contest in the live circuit (`landContest` from `crownTestimony`, a `disputedBy` branch in the ledger block). 21: derivation and recourse live (a `/declare` door for the giver, `carry: true`, `exposure` before `concede`; no fragility statistic until the layer-size-matched null exists). 22: the terminal assay through the real browser — the first legitimate product completion claim. 23: the pre-registered grain-leak prediction, one script. 24: the documents reconciled. 25: the unwired census as a ratchet. **Refused by the program:** no new floor, order, stance, cell or organ because the map offers one; no corroboration-throughput tuning and no new paraphrase mechanism (each refused at its seam); no log-odds gate (LR = 1); no prompt campaign (P2); no Phase 4 migration; no non-text yet; no silent prefix, fixture skip, vacuous guard or unread number.

**Suites, failure names diffed.** the-fold 1,581 → 1,588 tests, 1,578 → 1,588 pass, 3 → 0 fail (the three names above left; `corroboration-report.test.mjs` ×3 and `reader-frame.test.mjs` ×4 joined). eoreader7 native 619 / 618 / 1 todo before and after; organs 449 / 449 before and after. `mine-1-official-graph` re-run: 105 essays, 7,408 nodes, 3,873 edges, 0 graph-empty — its committed export predates the wipe and was left as the dated record it is.

## P97 — The finish line, executable: one answer record per question before any mouth speaks, and two walls the live circuit cannot yet hold (2026-09-05)

**Generality:** universal for the rule (the product bar is a runnable object whose zero-call walls a test reads on every suite run, and a wall that breaches is reported with its mechanism, never tuned until it holds) and for the two mechanisms named below (each replays on any English prose the production reader is handed); every count is the built corpus's own and dated. eoreader7 **S67** carries the driver, the lib and the test.

**What "there" is, as code.** `eoreader7/native/eval/the-fold/lib/product-assay.mjs` builds a corpus by construction — a fact stated once, a fact stated twice, two links of a chain whose consequence nobody states, and one claim a second source denies in its own bytes — and runs the production organs a live turn stands on (the relation reader with the app's own `RELATION_READER_OPTIONS` key for key, the hyperlexicon door, the per-source testimony spine through the grid, derivation from a declared giver, the contest act) to produce an **AnswerRecord** per question BEFORE any generation: `{claims[{claim_id, end1, label, end2, verdict, polarity, refs, spans[{ref,start,end,text,verified}]}], standing[{…, sources, phrase}], derived[{…, premises, restsOn}], contests, frame, recipe, sources[{name, sha256}], constitutions[{name, sha256 | gap}], declarations}`. Twelve walls. `tests/product-assay.test.js` reads ten of them every run and logs the other two.

**Ten held on the first run, unturned.** Configuration (the frame derived from the reader's options, P96; a recipe minted; both constitutions resolved by content hash — `FOLD-CONSTITUTION.md` lives outside the-fold repo, and the record now carries a typed `constitution_unresolved` gap when a checkout lacks it instead of a folded prompt with no drill-down). Address: 21 of 21 claim spans resolve to the source's exact bytes in source coordinates. Stated-once: the founding reaches the record at one source phrased "stated once so far, nowhere else yet" beside the repair at two. Derivation: Rowan → Owen derived, never stated, resting on two premises at one source each; zero derived without a giver. The record carries it; the reader REFUSES the same sentence as a draft (unbound) — derived ground and material ground are two marks, never one. Contest on the record: one CON·Figure·CONTESTED landed from the denying source's own bytes, kind `contest`, leak assay byte-identical, derivation reporting the contested premise rather than withholding it, and the disputed question's record saying so. Determinism: two builds byte-identical; the object-deranged corpus shares 0 of 6 claim ids. Recourse: `exposure` names one product before the act, `concedePremise` withdraws exactly that one, the fold no longer carries it, and the ledger grew 10 → 12 entries (nothing removed).

**One wall was the fold, not the circuit.** `derivation.js::foldDerived` projected every field of a derived entry except `restsOn` — the field P89 says every product records — so a reader of the fold saw a derived fact with no fragility. Found by wall 3 on the first run; one line, projected now.

**Two walls breach, by named mechanism, and are logged rather than pinned** (P95: asserting a known gap freezes it as a target). **(4a) The spine cannot see the disagreement.** "The Northgate Observatory never opened in 1889" is on the record as a bound edge with polarity `-`, and the per-source spine still reads the claim "opened in 1889" as `holds` from A and `undetermined` from B → SINGLE, never DISAGREE. Mechanism: a negated statement never yields a `contradicted` verdict under the production configuration — `negationBeforeVerbFor` reads "did not X" as act `did` with the negation leading the object (P43's own inverted shape), and DR5 folds "never X" into the act itself, so the draft's act `opened` never meets the source's `never opened`; `mergeTestimony` cannot see a refusal it was never handed. So the only live path to a contest today is a model witness (`/corroborate`, P88) — a model call. **(6) One shared token binds a fabrication.** "Amelia Hartley founded the Royal Society in 1887" comes back BOUND against a source that says only "…founded the Northgate Observatory in 1887": below `CORPUS_MINIMUM` the object match is `tokensShare`, hypergraph.js's own disclosed fallback, and a live turn's retrieved passages are always sub-floor. A mouth that keeps the subject, the act and one object token is marked material-ground. Three of four fabrications are refused (a bakery; nine comets; the derived-only Rowan → Owen).

**The model arm, spent once and read by no test.** gemma2:2b, 13 calls, the three questions through the real `runHolonicTask` with the sources as passages — the shape a live turn has today. "Rowan Vale was the founder of the Northgate Observatory… He preceded Marta Quill, who was the founder" — two fabrications in one line where the record carries one derived fact with its two premises; "opened in 1889" with no word of the denial, because the headless call carried no ledger. That transcript is why the next pass hands the mouth the record and not the passages.

**What this closes and what it opens.** Pass 16 closed. Two blockers for the terminal assay (Pass 22) are now named with mechanisms, and neither is a tuning: contradicted verdicts on negated prose (P43's inversion at the reader; the record is ready, the reader is not) and the sub-floor object match (a fabrication wall in the draft checker). Both are draft-side checks in Pass 19's territory; the record-side walls all hold. **Suites:** eoreader7 native 619 → 626 tests (625 pass, 1 todo), organs 449 / 449; the-fold unchanged at 1,588.

## P98 — One durable reading record: the ledger, the grid and the metacognition log persist append-only and replay through the kernel's own append (2026-09-05)

**Generality:** universal for the rule (every kernel task-log this surface holds is persisted as an append-only sequence of sealed entries and rebuilt by replaying them through `append`, so a reload reproduces the record by value; a hole, a bad line, or a missing source is a typed gap, never a silently shorter reading) — structural, no tuned number; the live counts below are one session's and dated.

**The defect.** `state.hyperlexiconLog`, `state.gridLog` and `state.metaLedger` were all born fresh on every page load — app.js said so in three comments ("a fresh page load is a fresh ledger"). Everything the ledger block stands on (P84) ended at reload; "runs for months" was not a property the record had. Meanwhile the sources DID persist (OPFS, `sources-store.js`), so a reload restored the material and forgot the reading of it.

**What ships.** All three logs are one kind of thing — a kernel task log (`{entries, nextSeq, admits}`, each entry sealed with its `seq`) — so one mechanism covers them. `record-log.js` (pure): `serializeRecord(log, fromSeq)` → JSON lines; `replayRecord(lines, {createTaskLog, append, admits})` rebuilds through the kernel's own `append` (a replayed entry is sealed exactly as the live one was) and returns a typed `record_gap` / `record_unparseable` / `record_refused` with the line number rather than a shorter log; `recordIdentity(log)` is sha256 over the entries in CANONICAL form (keys sorted at every depth — `append` seals `{...entry, seq, depends_on, evidence}`, so a faithful replay differs from the live log in key ORDER and in nothing else; an identity that read key order would call a faithful reload a different record — found by the test's first run); `resolveAddress(at, sources)` returns the bytes a `name#start-end` names or a typed `source_absent` gap — removing a source removes bytes, never history (FOLD-CONSTITUTION I.5). `record-store.js` (OPFS): one JSONL per log under `records/`, `appendRecord` seeks to the end and writes only the new lines (`keepExistingData`), nothing here truncates; `clearRecords` exists for the person and the app never calls it. `app.js`: `syncRecords()` after every site that assigns one of the three logs (nine sites — the grounded turn's write-back, `/corroborate`, Ranke, `/act`, crownTestimony, the terminal bridge, the S1/S2 observation), chained so appends land in order; `restoreRecords()` at boot after the sources load, each log restored only if it replayed something, a gap logged by name. `sources-store.js` records each source's sha256 in its index, so a record's addresses can say which bytes they named.

**Pinned** (`record-log.test.mjs`, 6 cases, against the REAL ledger organ over the REAL kernel log, with a frame and a landed dispute on it): fold(replay(serialize(log))) is byte-identical to fold(log), the identity matches, the contest and the frame survive; **a control that can fail, and does** — one entry mutated is a different identity and a different fold; a hole is `record_gap` naming the expected and found seq; incremental serialization concatenates to the whole; an absent source is `source_absent`; the grid's admitted operator set survives.

**Verified live, in the real page, against a real model.** Attach the assay corpus as `pasted.txt`; ask "Who founded the Northgate Observatory?" (gemma2:2b answers from the material); OPFS holds `records/hyperlexicon.jsonl` with 5 lines — the DEF·Ground frame and four notes — and the source index carries the sha256. **Reload.** The boot console reads `record restored: hyperlexicon 5`; the file still holds 5 lines (no duplication). Ask "When did the Northgate Observatory open?" with the material attached: the ledger block is absent from the prompt because the fact block already carries those notes (holon.js drops lines the fact block holds — by design). Turn attachments OFF and ask again: the prompt now carries *"From earlier reading, stated once so far and bearing on this question — one account's claim, not a settled one: The Northgate Observatory — opened→ in 1889 (stated once so far, nowhere else yet)"* and the answer is *"Based on the information we have, 1889."* — from the record, after a reload, with no material in the turn. `grid.jsonl` grew to 2 lines from crownTestimony's evaluate acts. That is bar item (2)'s "accumulated" half, which P84 had only in memory.

**Disclosed.** The frame entry replays as it was; a reader whose configuration changed since redeclares it on the next grounded turn (P81), so a restored ledger never carries a stale frame as current. `state.reflexLog` (per conversation) is not persisted here — it is the self plane's and has its own conversation store. The record is per origin (OPFS): another browser is another reader. Pass 18 will give the record a reading CURSOR so a source read on arrival is not re-read on every boot.

**Suites.** the-fold 1,588 → 1,594 tests, 0 fail throughout. No engine change.

## P99 — Read when material arrives: one admission loop for the turn and the background read, a cursor that resumes, an unread extent the question is told about (2026-09-05)

**Generality:** universal for the rules (a source is read on arrival through the same door a turn admits into, by one implementation; the reading cursor is the count of passages admitted under a recipe and a different recipe is a different instrument; a question asked mid-read is told the typed unread extent; two writers from one base MERGE, never overwrite) — structural, no tuned number; the timings are one machine's and dated.

**The defect.** `addSource` chunked a source and persisted it and read none of it. Cast, relations and the hyperlexicon door ran only per turn, over the three passages retrieval handed the model, so a book attached and never asked about was a book never read — and the ledger the record now keeps (P98) held only what questions had happened to touch.

**What ships.** `read-on-arrival.js` (pure): `admitPassages` is holon.js's per-part admission loop moved out VERBATIM (ledger born on the first bound edge, frame redeclared on drift, witness `<ref>~<recipe>`, refusals returned never discarded) and holon.js now calls it — one implementation, so the turn path and the arrival path cannot drift; `readOnArrival` is the loop: the production relation reader built ONCE over the source's own passages as its pool (declared in the result), one passage per macrotask, each passage admitted onto the ledger OF THE MOMENT (`ledgerRef`) so a turn landing mid-read is never overwritten; `unreadExtent` is the typed extent a question is told about. `app.js`: `readSourceOnArrival` runs after the received priors settle (`priorsSettled` — the three prior fetches are awaited so the recipe read under is the reader's settled configuration, not a boot-time frame that would differ on the next load), on every `addSource`, and on boot from each source's saved cursor; `hyperlexiconUnread` reaches `runHolonicTask`, and holon's ledger block says *"Still reading: name — k of n passages so far. What follows is from the part already read."* even when no note matches. `sources-store.js` keeps a row's cursor and recipe across a rewrite of the same bytes and gains `updateSourceMeta`. The turn's write-back and the two doors (`/corroborate`, Ranke) merge through `record-log.js::mergeAppendOnly` — every entry either chain appended beyond the shared base is re-appended through the kernel's own `append`, a note heard by both is the door's idempotent no-op — instead of overwriting.

**Pinned** (`read-on-arrival.test.mjs`, 5 cases, real door and real production reader over the assay corpus): every passage admitted in order with notes on the ledger before any question; a read resumed from its cursor is byte-identical to a straight read (fold and `recordIdentity`); a suffix never alters or removes a prefix note's own witnesses and spans and may only ADD a witness — with the control that must fail (a "suffix" that concedes a prefix note removes it, and the assertion sees it); a second recipe is a second instrument (sources 1, instruments 2), the same recipe re-read changes nothing; an exhausted cursor reads nothing and says so. `record-log.test.mjs` grew the merge case (both writers' entries survive, a note heard by both is one note).

**Three things the live run found, kept as rules.** (1) **A hidden tab clamps `setTimeout` to about one tick a second** (and to one a minute after five minutes), so a 44-passage read that yields per passage crawled — the page's yield is now a `MessageChannel` macrotask, which is not clamped and still lets the page paint. (2) **Serialize at job time, not call time.** The first `syncRecords` computed the lines to append when called; two calls issued before the first append finished captured the same seqs, and the OPFS file held a duplicated stretch — reported on the next boot by replay's own typed `record_gap` at line 703 (the enforcement finding the defect, P94's law applied to the record). The job now reads the log and the cursor when it runs; replay counts and skips a byte-identical repeat (`duplicates`) and types a repeat with different bytes as `record_conflict`. (3) **Boot must not re-persist a source it just loaded from OPFS** — the rewrite raced the cursor's own row and dropped it; a row now keeps its fields across a same-bytes rewrite and boot skips the write.

**Measured live, in the real page.** `pasted.txt` (174 B): 1 passage, 4 notes, 59 ms. A 143 KB slice of War and Peace pasted as `pasted-2.txt`: 44 passages (the app's outline boundaries cut ~3 KB passages where `chunkSource`'s default cuts 634 of ~230 chars), read in **31.3 s**, 698 notes heard, 0 turned away, cursor 44 persisted; the record grew 5 → 703 lines; a question asked two passages in carried *"Still reading: pasted-2.txt — 2 of 44 passages so far"* to the model. Reload: `record restored: hyperlexicon 703, grid 2`, no re-read (the cursor is honored), and the recipe matched. Headless, the production reader reads a 230-char passage in 2–21 ms and builds over 634 passages in 3.7 s; the whole-book arrival read of `pg2600.txt` is running as this entry lands and its number is appended below.

**Disclosed.** The reader's pool is the whole source, so the vocabulary a late chunk contributes can reach an early chunk's reading — resumption is invariant by construction (the reader is rebuilt over the same pool), prefix-invariance under a *changed* pool is not claimed. Per-turn admission still runs over retrieved passages (idempotent by witness, so nothing doubles). The arrival read's admission gate is the same `connectorLens` a turn uses and is off when checking is off. The record grows by the whole reading now — 703 lines for 143 KB — which is what Pass 17 was for.

**Suites.** the-fold 1,594 → 1,600 tests, 0 fail; no engine change.

## P100 — Claims before prose: the AnswerRecord, the specificity wall at the reader, the marks painted, and bar item 5 restated by measurement (2026-09-05)

**Generality:** universal for the rules (every turn writes an AnswerRecord — what was handed, what was said with the verdict the material gave it, what nothing backs, the reader's identity — to the durable record before the prose is trusted; a claim binds only to an edge that states its whole object; a model swap is compared on the record, never on the prose); every count is one session's and dated. eoreader7 **S68** carries the judge change and the diff driver.

**What ships.** (1) `objectSpecificity` in the reader (S68), ON in `RELATION_READER_OPTIONS` — the assay's wall 6 holds, 4/4 fabricated drafts refused, and the same wall now stands in the live draft checker and the arrival read. (2) `answer-record.js` (pure, 3 tests): `answerRecord` folds a turn's sections into `{claims[{key, end1, label, end2, verdict, polarity, refs, spans}], tally, unsupported, unbacked, retrieved, unread, sources, constitution, frame, recipe, model, cursor}`; `claimKey` is the identity a diff compares on; `diffRecords` reports shared / only-A / only-B record-backed claims and the count of claims nothing backs, never averaged. `app.js` builds one per grounded turn from `result.sections`, appends it to `records/answers.jsonl` (append-only, beside the three logs of P98) and renders it FIRST in the thinking panel with its one-line summary; the folded constitution's sha256 rides every record. (3) The standing `body.marks-off` suppression is lifted: a checked turn paints its address chips, ground underlines, relation badges and the witness's ∅ unless `fold-marks` is off — bar item (1) on the surface again. (4) A fast-pass (S1) answer is classified like every other turn: nothing was handed to it, so every sentence is model-ground and wears the dotted underline instead of shipping unclassified.

**Bar item 5, measured and restated.** `model-swap-diff.mjs` ran the real turn over the assay corpus under gemma2:2b and llama3.2:latest: nothing-backs = 0 held for neither model (llama3.2 placed the observatory "in Northgate, Edinburgh, Scotland"; gemma2 wrote "Rowan Vale also"), and the record-backed claim sets were identical on 0 of 3 questions — each mouth chose different true things to say. "Swapping the model changes only phrasing" is not what two small mouths do, and the number that would make it so is not one to tune toward. What IS model-independent is the RECORD — the ledger, the retrieved claims, the frame and recipe — by construction, and the record now says exactly what each mouth added beyond it. **Restated:** the record is identical across models; every sentence a mouth adds beyond the record is marked model-ground and counted; the record-backed sets are reported side by side. The terminal assay (Pass 22) inherits this wording.

**Verified live, in the real page.** A grounded turn now paints its marks (an address chip on the bound sentence; ∅ and dotted underlines on model-ground sentences); the thinking panel opens on `answer record · 8 claim(s) (3 unbound, 3 beyond-reach, 2 unheard) · 0 unsupported · 3 unbacked · retrieved 3 · recipe 0f2a4681eccd` followed by the record and the four calls; `records/answers.jsonl` holds the line. Flipping the reader option changed the recipe, and both sources were re-read as a second instrument on the next boot exactly as P99 designed (143 KB in 29.6 s; the record grew to 1,389 lines).

**Two findings the record made countable, handed forward, not fixed.** (a) **Retrieval by term hits prefers long passages.** "When did the Northgate Observatory open?" retrieved `pasted.txt#0-174` and two 3 KB War and Peace passages; the mouth wrote about Pierre and Anatole; the record read 0 bound, 3 unbacked, every sentence marked. `retrieve()`'s zero-relevance floor (P4) and the app's outline-grain chunks together let a long unrelated passage outrank a short right one. Named for the next pass that touches retrieval; the marks and the record already say the answer stood on nothing. (b) **An adverb absorbed into the subject** (`rowan vale also|preceded|…`, `the northgate observatory actually|opened|…`) makes two keys of one claim — a reading-grain finding for the extractor, not for the diff.

**Also measured, whole-book reading (P99's pending number).** At the app's outline grain, a 600 KB slice of War and Peace (186 passages, avg 3.2 KB) reads in 245 s — of which the relation reader's build over the pool is ~96 s and the passages ~0.8 s each — 3,769 notes, 0 corroborated (a novel does not restate itself). The whole 3.3 MB book (1,051 passages) has been building its reader for over 20 minutes at 99% CPU as this entry lands: **the pool build is superlinear**, so a whole-book arrival read is not practical in the page today. Next step for the arrival reader, named: build the reader over a bounded pool (the outline section, the chapter) and let the ledger carry the joins — the same lesson `cast.js` learned for atmosphere (hop = window). Not attempted here.

**Suites.** the-fold 1,601 → 1,604 tests, 0 fail; eoreader7 native 626 / 625 / 1 todo, organs 449 / 449.

## P101 — Contest in the live circuit: a DISAGREE lands on the record from the turn's own testimony spine, and a disputed note says so to the mouth (2026-09-05)

**Generality:** universal for the rules (the per-source testimony spine's DISAGREE / CONTRADICTED lands CON·Figure·CONTESTED through `landContest` with the refusing source's own bytes as decider and never convicts; a note under a live dispute is phrased to the mouth as disputed and unsettled, never withheld and never convicted) — structural, no tuned number; the live run's counts are one session's and dated.

**What ships.** `app.js::crownTestimony` — after `mergeTestimony`, a DISAGREE or CONTRADICTED merge calls `landContest(ledger, hyperlexiconFor, merged, { textAt })` (the wire P91 named and no app caller had), `textAt` slicing the refusing reading's address from the source's own bytes; the log is written back and synced; the crown line reads "(recorded as a contest, not settled)" and the disclosure carries the landed count and every typed refusal (`self_witness`, `read_nothing`, `no_bytes`, `not_a_contest`). `holon.js::ledgerBlock`'s `phrase()` gains the branch `foldWithStanding` had carried unread since P88: a note with `disputedBy` is said as *"…; disputed by <source> — not settled"*. Pinned in `holon.test.mjs` (the P84 ledger pin now carries a disputed note and asserts the phrase reaches the model).

**Verified where it can be, disclosed where it cannot.** The record side is proven by the product assay (walls 4b and 4c: one CONTESTED landed from the denying source's bytes, leak assay byte-identical, the disputed question's record says so) and the mouth side by the holon pin. **No live contest has yet been produced by either path, and the reasons are on the record:** the spine reads a negated denial as `unbound`, never `contradicted` (P43's inversion, the assay's wall 4a), so `mergeTestimony` never sees a refusal to land; and the `/corroborate` walk over the three loaded sources (`pasted.txt`, the War and Peace slice, `northgate-b.txt` with "never opened in 1889") spent 1 of 8 asks, skipped 21 pairs at the co-presence gate, attested 0 and contradicted 0 — the model witness was never asked the denying pair. The wire is live and waits on a reader that can read a denial; that reader is the named next lever, not a tuning of this one. The P96 render read "contradicted: 0 (landed as disputes: 0)" without throwing, which is what P96 was for.

**Suites.** the-fold 1,604 tests, 0 fail. No engine change.

## P102 — Derivation and recourse in the live circuit: a declared giver, a derived tier the mouth is told, and a concession that shows what falls before it falls (2026-09-05)

**Generality:** universal for the rules (nothing is derived from an undeclared relation and every derived fact names its giver; the mouth is told a derived fact as derived — never stated, resting on N claims, the weakest stated once — never as a settled thing; a concession is previewed by `exposure` before `concedePremise` performs it, and the record keeps every entry) — structural, no tuned number; the live counts are one session's and dated.

**What ships.** `app.js` holds a declarations register (`interpretation/declarations.js`, the fourth persisted log — `records/declarations.jsonl`, replayed on boot like the other three) and a derivation bundle over the same ledger (`makeDerivation({ hl: hyperlexiconFor, taskLog })`). Three doors: **`/declare <rel> transitive`** or **`/declare <rel> composes <product>`** proposes and promotes a declaration with the giver `person:chat (declared in this conversation, <date>)` — the person is the giver, named on every product (II.2); bare `/declare` lists the register. **`/derive [maxSteps]`** runs `derive` with `carry: true` at the honest floor `{sources: 1, instruments: 0}` (P89) and prints each live product with its depth, premise count, weakest source count, giver and premise ids; the result merges onto the ledger (`mergeAppendOnly`) and syncs. **`/concede <id>`** shows `exposure` — what would fall, with cascade depth — and **`/concede! <id>`** performs `concedePremise` with the trigger recorded. `holon.js`'s ledger block gains a third tier, ranked by the question like the others: *"From earlier reading, derived — no source states these; each follows from claims already read, and falls with them: … (follows from 2 earlier claims, never stated itself; the weakest of them stated once so far)"* — pinned in `holon.test.mjs`. `eoreader-contract.json` records the new engine import.

**Verified live, in the real page, the whole loop.** `/declare preceded transitive` → declared with its giver. `/derive` → *"Rowan Vale —preceded→ Owen Blythe · depth 1 · rests on 2 claim(s), weakest at 1 source(s) · premises: rowan vale|preceded|marta quill ; marta quill|preceded|owen blythe"* — 705 premises on the ledger (the War and Peace slice's notes among them, all carried), 40 withheld, quiescent. Ask *"Did Rowan Vale precede Owen Blythe?"* → the prompt carries the derived tier verbatim and the answer is *"Yes, Rowan Vale preceded Owen Blythe."* — with the witness's ∅ beside it, because no passage states it: a derived fact wears a material chip AND a witness refusal today, and that disagreement is the finding below. `/concede rowan vale|preceded|marta quill` → *"would withdraw 1 derived fact(s) (cascade depth 1): Rowan Vale —preceded→ Owen Blythe"*. `/concede! …` → withdrawn 1; `/derive` → 0 live, 704 premises. **Reload** → `record restored: hyperlexicon …, grid 30, declarations 2`; `/declare` lists the declaration; `/derive` → 0 live. The concession survived the reload; the record kept every entry (`hyperlexicon.jsonl` 1,653 lines, nothing removed).
...
## P104 — Negation is a cut, not a sign: a denial is heard apart from the link it denies, the contest lands at the door, and both are tracked through time (2026-09-05)

**Generality:** universal for the rules (a denying source is never a witness of the link it denies; a cut meeting its link lands an open contest with the denying bytes as decider and never convicts; every act touching a claim is an event with a seq, and the reading of a denial is its timeline, never its present state). The reader's negation fold is English-tagged (received `negationWords`). Counts are the assay's own and dated. eoreader7 **S69** is the paired entry; `eoreader7/native/docs/THE-NULL-STATES.md` the nomination.

**The user's direction, and what it changed.** "Polarity deserves deeper thinking of the math involved — SEG and CON seem most tied to polarity"; "we can also DEF things as VOID"; "look at all the possible null states we can define, this is our true source of wisdom"; and, during the build, "remember to track negation and void through time." The first repair of P103's reader miss had folded "never X" into act X with polarity `-` and shipped the denial as a witness of the link — the door then refused the contest as `source_already_witnesses`. The grid of nulls named the cell that was empty: SEG·Figure. A denial is a subject separated from an extent; it shares the link's ends and never its id; it is disputable; it is a veto and never a premise.

**What ships (engine, S69) and what changed here.** `read-on-arrival.js` carries `polarity` on every admitted edge, so the arrival read lands a denying passage as a cut and the door lands the contest the moment the cut meets its link — in either order, across sources, without a model. The kernel's `negationTimeline(log, linkId)` is the reading of a denial through time: link heard at seq, cut heard at seq, contest at seq, settled or conceded at seq, with a standing for each; a conceded cut leaves the fold and stays in the timeline. The void (DEF·Ground) is designed to the same shape — an event with a cursor over an extent, re-zeroed by one arrival — and is the next object, not built here.

**Measured.** Product assay 12/12 walls held; wall 4b landed at the door and not doubled by the spine; standing byte-identical across the acts. The-fold suite 1,604 / 0 fail. eoreader7 native 629 / 628 / 1 todo; organs 449 / 449.

**Verified live, in the real page (the first contest ever produced by the circuit itself).** Two sources dropped onto the composer — `obs-a.txt` ("The Northgate Observatory opened in 1889. …") and `obs-b.txt` ("The Northgate Observatory never opened in 1889. …") — were read on arrival; the record shows the link heard again (seq 1397), the cut born as SEG·Figure (1399), and CON·Figure·CONTESTED landed at the door (1400) with the denying sentence as decider and its byte address; the older `northgate-b.txt` was re-read as a second instrument and landed a second contest (1403). Replayed through the production fold, the note reads `sources 2, disputedBy [obs-b.txt, northgate-b.txt]`, and `negationTimeline` reads link → cut → contest → contest with the standing `{link: live, cut: live, contest: open}`. The question "Did the Northgate Observatory open in 1889?" then carried, in the ledger block, *"The Northgate Observatory — opened→ in 1889 (read in 2 places; disputed by obs-b.txt, northgate-b.txt — not settled)"*, and gemma2:2b answered *"The sources are conflicting. … It appears the two sides of this argument are not settled."* quoting both addresses. Two residues, named: the FIRST question after the drop carried the note without the dispute (the second, seconds later, carried it — a turn-snapshot timing not yet pinned); and the pre-cut record still holds the old reading `never opened→ in 1889` as a stated-once LINK under the earlier recipe — an honest record of what that reader heard, which the timeline distinguishes from the cut and which no consumer yet phrases as superseded.

**Handed forward.** The void as a first-class object with scope and its own timeline; a derived sentence's own ground mark; the live page's first contest produced by the arrival read, with `/corroborate`'s render showing it; retrieval preferring long passages; the bounded reader pool.

## P105 — The void in the live circuit: declared with its scope before the mouth drafts, relayed as an open gap, cancelled by the first filling, withdrawable, tracked through time (2026-09-05)

**Generality:** universal for the rules (a void the reader declares carries its scope — which sources, how far read, the ledger's cursor — and is refused without one; the mouth is told a declared void as "looked for and not found so far … an open gap, not a finding that it is false", never as a conviction and never as its own finding; the first filling re-zeros it on the record by name; a void is withdrawn with a recorded trigger, never deleted; every act is at its seq). The slot reader that names a void's ends is the void-brief's (English possessive / adposition shapes, P54's own limits). eoreader7 **S70** is the paired entry; THE-NULL-STATES the nomination.

**What ships.** `app.js`: at the material moment of the void brief — material in hand, the model not yet drafting — an empty slot (no fillers; the head phrase not collapsed onto the anchor; the brief's year-extent either holed or undeclared, since the read scope is the denominator either way) lands `declareVoid` on the ledger with `scope: {sources, read, total, cursor}` and the brief's own reason as `because`; the brief's fillers, when a later turn finds them for an OPEN void, land `rezeroVoid` by name with the filler's witness; `hyperlexiconVoids: voidsNow()` hands live voids to the turn. `holon.js::ledgerBlock` gains the fourth tier, ranked by the question like the others: *"Looked for and not found so far — say these are open, never that they are false: - <end1> — <label>→ ? (looked for in N sources, all of it read | k of n parts read so far; an open gap, not a finding that it is false)"* — pinned in `holon.test.mjs`. **`/void`** lists open gaps with scope and seq, **`/void <id>`** shows one timeline, **`/void! <id>`** withdraws with a recorded trigger.

**Verified live, in the real page.** "Who was the Northgate Observatory's director?" over five attached sources landed `void:northgate observatory|director|*` — *looked for in 5 source(s), 48 of 48 parts read · reached true · declared@1407* — and the mouth answered that it could not. The next question carried the tier verbatim in the ledger block and gemma2:2b answered *"The Northgate Observatory director's identity is not yet clear. It's an open question."* Before that, the first live run declared a wrong-shaped void (`Northgate Observatory —northgate observatory→ ?`: the slot reader collapsed "the director of the Northgate Observatory" onto its anchor, P54's own reader gap) — caught by `/void`, withdrawn by `/void!` (`declared@1404 → redeclared@1405 → conceded@1406`), and the head-equals-anchor guard now refuses that shape.

**Residues, named.** The door's own re-zero fires on an arrangement whose LABEL matches the void's (proven by the assay: `closed`); a slot-shaped void (`director`) is filled live only through the brief's filler organ, which today knows succession boxes — so a plain sentence "Amelia Hartley directed the observatory" does not yet cancel that void. The year-extent dimension of the brief is not carried onto the kernel void. The thinking panel's live lines about declaring and filling are ticker-only (P54's simplification) — the record and `/void` are where to look.

**Suites.** the-fold 1,604 / 0 fail (`holon.test.mjs` 65); eoreader7 native 633 / 632 / 1 todo; organs 449 / 449; product assay 13/13.

## P106 — Every ∅ cites its void: an absence the answer asserts is honest when a declared gap is in scope for it, and a counted leak when none is (2026-09-05)

**Generality:** universal for the rule (an absence sentence — witness-refused or unbacked — either cites a void the reader DECLARED, with its scope, or is counted as the mouth declaring emptiness; the match is mechanical: the void's first end named in the sentence or in the question it answers, the void's label in the sentence itself; nothing reads meaning) — structural, no tuned number; the live run is one session's and dated. Pass 25 of the null experiments (NEXT-PASSES); eoreader7's model-swap diff prints the same tally per mouth.

**What ships.** `answer-record.js::voidInScope(sentence, voids, {question})` and `absencesOf` — the AnswerRecord carries `absences[]` (sentence, how, void id, scope) and `absenceTally {citingVoid, citingNone}` and `voidsOpen`; `answerRecordLine` says "absences N (k cite a declared gap, m cite none)". `app.js` hands the record the open voids and the witness rows, keeps `state.lastAsked`, and the ∅ badge on a witness-refused sentence reads *"∅ open gap on the record: <end1> —<label>→ ?"* (title: the void id, its scope, "cancelled by the first arrival that fills it") when a void is in scope, and the old "∅ no passage states this" — now saying "and no declared gap is in scope for it" — when none is. Pinned in `answer-record.test.mjs`: the P54 leak sentence cites nothing; a planted absence with no void declared is counted (the control fails as built); an anaphoric absence is anchored by the question, never by the sentence alone.
...
## P108 — Long-form: a piece asked for by its length is planned by the number, each section told its place and word target, a short section continued once by measurement (2026-09-05)

**Generality:** universal for the rules (a writing ask with a stated length — pages or words — is work with a declared size, routed before every material detector; the section count comes from the number, never from appetite; the material is found by a topic-anchored hunt before the plan and unioned with what is attached; every section is told its place in the piece, the outline, how the previous section ended and its word target; a section under `CONTINUE_BELOW` of its target is continued exactly once, measured, before any check reads it; progress and completion land on the record as `longform-part` / `longform-done`). The one number that is declared and unmeasured is `CONTINUE_BELOW = 0.6` — its own comment says so. Counts below are one session's and dated.

**What ships.** `longform.js` (pure: `detectLongForm`, `longFormTask`, the size constants with their givers) + `longform.test.mjs`; `app.js`: the plain ask ("write me a 30 page essay on the x files") routes to `longFormTurn`, `/essay <pages> <topic>` is the typed door, the preflight hunt is forced for a piece with the topic verbatim as its query (the query builder folds "The X-Files" to "Files"), `LONGFORM_PAGES_CONSULTED = 8` is the leash, the void brief stands down for a piece (it read the planning sentence as a slot question and declared a void over it — the first run), and the turn mirrors `longform-part` / `longform-done`; `holon.js`: `executeMaxTokens`, `planMaxTokens`, `piece` options, `pieceLine`, `wordCount`, `tailWords`, the one-shot continuation before inspection, `continued` on the part; `explore-server.mjs`: the record mirror's allowlist now names every chat door (seven had been refused 400 for days, silently) and the two long-form events.
...
## P110 — The piece's own checks: the unconscious reads the terrains, the mouth hears sentences (2026-09-05)

**Generality:** universal for the rules (a section is told, as plain sentences, what the instrument computed from the material — the cast it should say something about, what the piece already said, what the sources disagree on and what nothing read says — and never a terrain's name; a section whose bound claims are all already on the piece's claim set is asked once for something new; a section naming none of its obligations is asked once for them; sentences that echo the instrument's own asks are cut by construction — words of the ask the material never uses, two in one sentence; a section whose own retrieval is thin hunts on its own words under a small leash; for a piece the correction ladder marks `unsupported`, `incomplete` and `narrated` and keeps the prose — copying and echo are still corrected; the search digest lands on the record as bytes). The word target and the leashes are declared (`WORDS_PER_SECTION`, `LONGFORM_SECTION_PAGES = 2`); nothing is tuned. Counts come from run 3 and go in the amendment.

**Where each terrain enters, and what the model sees.** Entities: `obligationsFrom(referentIndexFor(passages))` — the cast organ's referents ranked by recurrence → "This section should say something about: …"; coverage is measured after the draft (`coverageOf`). Links: the piece's claim set, unioned from every finished section's bound claims → "Earlier sections already said: …", and the duplicate veto. Paradigm: the last section is handed the open contests (`disputesOf`) and the declared voids → "The sources disagree on: …" / "Nothing read says: …". Void: a thin section's own hunt. Kinds, atmosphere and the network are named and not yet wired: the outline is still the model's, grouped by nothing; section boundaries do not yet follow the source's own re-zeros; the order is the model's. The instrument hands the model its facts and measures; it does not author the outline — the decision P108's question left open, taken here as the narrower one.

**What run 2 showed and this closes.** Fourteen of eighteen sections under 120 words: the correction ladder rewriting 600 words of prose under a critique, and the mechanical fallback replacing it; "There is no need to restate the question" and "Let me know if you'd like me to continue writing this" shipped as prose; nine sections re-introduced the show; the hunt settled at two pages for the whole piece.

**Files.** `holon.js` (`pieceLine` grown; `obligationsFrom`, `coverageOf`, `cutMetaTalk`, `continueAsk`; the per-section hunt and obligations in `runPart`; `pieceMode`; the duplicate and coverage asks; `piece` on each part's result with `words`, `coverage`, `reasked`, `metaCut`, `hunted`), `app.js` (`referentIndexFor` and `huntFor` on the piece; P110's numbers on `longform-done`; the digest mirrored as `web-digest`), `explore-server.mjs` (allowlist). `holon.test.mjs` 67.

## P111 — The unconscious edits the mouth: a finished piece is edited model-free, every edit an act (2026-09-05)

**Generality:** universal for the rule (user direction: "the unconscious should be able to edit the model's output — we often edit things without consciously knowing why; the EVA need not be the model at all times"; so a piece, once its sections are written and checked, is edited by the instrument with no model call — a sentence that restates an earlier section is cut, a section emptied by its cuts is dropped, a section whose bound claims were all already said is merged into the previous one — and every edit lands as a SEG or SYN act with its reason, on the result and on the ticker, never a silent change; the restatement test is the same declared 8-word run the copy-check uses, never a similarity score; the meta-talk cut counts the instrument's own template words only — never the topic, the outline, the obligations — and only content words, by the received claim stoplist). Order stays the plan's own: the editor cuts and merges; reordering by the network is named, not wired.

**Files.** `piece-edit.js` (`editPiece`, `editLine`) + `piece-edit.test.mjs` (3, the control built to fail: a piece with nothing restated is untouched); `holon.js` (the editor at assembly for a piece; `INSTRUCTION_TEMPLATE` — the template alone is apparatus; `cutMetaTalk`'s rule: two apparatus words, or one in a sentence with no word of the material); `app.js` (edits shown on the ticker as `edited — …`, counted on `longform-done` as `edits` and `editKinds`). `holon.test.mjs` 68. **Found by the pin:** the first cut counted "this", "and", "where" as apparatus and emptied every section — function words are nobody's vocabulary; the stoplist is the wall.

**Amended same day — run 3 (P110 in, P111 not yet).** "write me a 30 page essay on the x files", the real page, gemma2:2b: 15 sections, **6,469 words** (run 2: 3,957; run 1: 2,269), every section between 249 and 585 words (run 2: fourteen of eighteen under 120) — the mark-not-rewrite posture is what stopped the shredding; obligation coverage 0.47; 4 re-asks (duplicate or coverage); 1 section's hunt found pages (the fandom wiki answers 403, so most per-section hunts came back empty); 3 unsupported, 57 unbacked marked; 21.5 minutes. `metaCut` read **141** sentences — that run carried the first cut rule, which counted "this", "and", "where" as apparatus (the P111 finding); real prose was cut with the meta-talk, so 6,469 is a floor for that reader, not its yield. Run 4, on P111 and the corrected rule, is the Borodino prompt.

## P112 — A program asked for by its shape: the piece machinery pointed at a build, with the sandbox run as the witness (2026-09-05)

**Generality:** universal for the rules (a building verb, a runtime the terminal's own registry names — `term.js`'s ROSTER, never a list typed here — and an enumerated spec make a program ask; the plan is one feature per part in build order; the first feature is born as one build and every later feature lands as a PATCH through the complaint door — scout, smallest edit, witness gate, record — never as a fresh fence; after every landing the sandbox runs the whole and the run's OUTCOME is the witness; a failed run is said back once as a fact with its own last line; coverage is measured against the spec's features by the code's declared names; a name declared twice is one more instruction to the fold; every step lands on the record as `codepiece-plan` / `codepiece-part` / `codepiece-done`). Numbers come from the first live run and go in the amendment. NEXT-PASSES Pass 30 is the plan entry.

**Files.** `longform.js` (`detectCodePiece`; aliases javascript→js, py→python, sqlite→sql), `app.js` (`codePieceTurn`, `runBuildOnce`; `foldTurn` gains `quiet` and `autoRun` options so the piece drives it without a user message per feature and without a second run), `explore-server.mjs` (allowlist). `longform.test.mjs` 4.

**Amended same day — run 4, a different prompt, on the editor.** "write me a 15 page essay on the Battle of Borodino" (7,500 words asked), gemma2:2b, the real page: 12 sections, **8,199 words shipped** — the first run to reach its asked length; every section between 430 and 774 words; `metaCut` 0 under the corrected rule (run 3's 141 were the first rule's own); 1 edit (a restated sentence cut); 2 re-asks; coverage 0.53; 0 hunts found pages; 0 unsupported, 119 unbacked marked; 19.5 minutes. The prose opens "The year is 1812…" and reads as an essay rather than a summary of one; the unbacked count is what a 2B mouth invents about a battle when four pages are what it read — marked, on the page, never absorbed.

## P113 — The self-review, measured: a code file is not prose, and a 2B mouth cannot review this instrument (2026-09-05)

**Generality:** universal for the rule (a source that is code by its name is retrievable, runnable and scouted for its declarations, and is never read into the ledger as English nor offered as a cast to a piece's obligations — the skip is typed on the reading map, never an unread extent); not-applicable for the run's numbers (one run, one small mouth, dated).

**The run.** "write me a 12 page report on improvements to The Fold, grounded in its own code and theory", web off, twelve of its own files attached (the five theory documents, `kernel/notes.js`, `holon.js`, the long-form and editor organs, `NEXT-PASSES.md`), gemma2:2b: 9 sections, 5,265 words, 0 unsupported, 41 unbacked, 2 edits, 11 minutes. **The piece is not a review.** Three mechanisms, each named by what it produced: (1) the prose reader read `.js` files as English — the ledger gained notes like `Arokin —is→ …` and the cast organ gave the introduction its obligations as "Abbé Morio, Abraham Lincoln VP, Andrew Johnson, Anna Pávlovna, Annette" (identifiers and example names out of `holon.js` and the plan), so the introduction became a list of people; (2) the sources attached hours earlier were still live — retrieval on the topic's few words drew the War and Peace slice and the X-Files pages, and "Security and Integrity" became a Borodino prose poem while "Theoretical Framework" explained The Fold through Agent Scully; (3) with nothing it could read as a claim about this instrument, the mouth wrote the report any software gets — "sophisticated algorithms", "well-organized menus", a "release package in 1997" — every one of those marked unbacked, correctly, and none of them a finding. The outline was the model's ("Performance Evaluation", "User Interface", "Applications"), the residue P110 already names.

**What ships.** `longform.js::isCodeSource` (by name); `app.js` — a code source is never read on arrival (typed `skipped: "code"` on the reading map, excluded from the unread extents); `holon.js` — a code passage is never admitted to the ledger by a turn, and a piece's obligations come from prose passages only. `longform.test.mjs` 5.

**What it does not ship, said plainly.** A review of this codebase needs a reader that reads code as code (declarations, imports, the tests that read a number) and a mouth that can hold an argument about it; the first is Pass 30's remainder and the second is not a 2B model. Material scoping per piece — only what was attached for this ask — is the next mechanical fix and is not built here.

## P114 — A piece stands on material in its scope; the outline is drawn from the sources' own sections; the register is the reader's (2026-09-05)

**Generality:** universal for the rules (a piece retrieves only over attached sources whose text carries every content word of its topic, plus what its own hunt finds, and says what it stands on and how much was set aside; the planner is handed the sources' own section headings as a fact — from the attached sources in scope and from the hunted pages — and asked for an argument that draws on them, never a copy; the register is a fact about the audience the mouth receives, not a rule the instrument trusts; every sentence keeps its byte address and the exported piece keeps, per section, the exact prompt it was generated from). Counts are one run's, dated. User direction that shaped it, verbatim: "ours use anchors to the source text unlike Claude", "every piece of generated content reveals what it was prompted", and the standing principle "a lot of thinking is unlocked from putting holonic objects in slots so you can losslessly refer to entire universes with a single address" — the addresses are the deliverable's spine, not its decoration.

**Files.** `longform.js` (`topicTerms`, `inScope`, `headingsOf`), `holon.js` (`planFacts` on the plan ask; `piece.register` on the piece line), `app.js` (scope computed before the hunt, said on the ticker once it exists — found live: `show` is declared after `live`; headings from attached in-scope sources and from the hunted pages). `longform.test.mjs` 6.

**Residue, named.** A one-word topic makes a weak scope: "the x files" folds to the single term "files", so the plan of record — which mentions these runs — read as in scope beside the fetched pages. The next rule is structural, not a threshold: a source is in scope when the topic's terms recur in it at the reader's own recurrence floor, the same floor the cast uses, or when its name carries them.

**Amended same day — run 5, the deliverable.** "write me a 30 page essay on the x files" on P114: scope set aside 100 passages of other material (and admitted the plan of record on the one-word term, the residue above); the planner, handed the fetched pages' own headings, returned 17 sections; **11,264 words shipped** (22.5 pages of 30), every section between 392 and 768 words; 2 unsupported, 69 unbacked marked, 1 restated sentence cut, 3 meta sentences cut, 16 minutes on gemma2:2b. Of 485 sentences, 19 carry a byte address, 84 carry the witness's refusal, the rest settled neither way — the anchoring rate is the reader's ceiling on a 2B mouth writing past its material, and the export (`xfiles-essay-college.md`) keeps every mark and, per section, the verbatim messages the model was sent.
...
**What was extended, where.**
- `arithmetic.js` — after the pure door (unchanged, byte-identical on every question it already answered): the shaped questions — unit conversion, N choose K, N factorial, a statistic over listed numbers, a derivative at a point, a linear or quadratic equation in one unknown — each detected by one whole-question regex and computed by the injected engine's OWN operation (`math.unit`, `combinations`, `factorial`, `mean`/`median`/`std`, `derivative`, `rationalize`'s coefficients; the quadratic root is `math.evaluate` over the formula as a string); and the calendar — days between two dates, the weekday of a date, a date N days from a date — whose engine is the host's proleptic-Gregorian `Date.UTC`, declared as such. `checkQuantity` runs the three in that order. arithmetic.test.mjs 23/23.
...
**The in-tab rung, wired.** P21's WebLLM rung had shipped as modules and tests (`webllm-rung.js`, `webllm-client.js`, `webllm-worker.js`, `webllm-rung.test.mjs`) that nothing in `app.js` imported: the page never offered it. It is wired now — `fillModels` appends the roster when `webgpuBlocker` is null (the blocker's own fix text lands in the status line otherwise), `connect` states the rung's declared window and where its bytes come from, `completeOnce` routes an in-tab id through `webllmClient.stream` with the same `{text, thinking, doneReason}` contract, the pace ledger fed from the engine's own telemetry. `mergeOffered`'s order stands: native rungs first where Ollama answers (measured live: the fold's summary refresh went to gemma2:2b while the answer ran in-tab), the roster alone where it does not, so a page served with no Ollama routes every kind in-tab (`webllm-rung.test.mjs`, 41/41). One engine lives at a time; picking another rung unloads the first.

**The roster, and why these three.** The vendored catalog (web-llm 0.2.84, 163 entries) holds no model trained only on licensed or public-domain text — the Common Corpus / Common Pile family would need an MLC compile of its own, named as the next step, not claimed. Among what it holds, the three whose publishers disclose the most:

| rung | publisher, licence | what is disclosed | model card |
|---|---|---|---|
| OLMo 2 1B Instruct (q4f16, 845 MB) | Ai2, Apache-2.0 | training data (Dolma / OLMo-mix), code, weights, training logs — all published; already this instrument's S1 rung under Ollama | huggingface.co/allenai/OLMo-2-0425-1B-Instruct |
| SmolLM2 1.7B Instruct (q4f16, 937 MB) | Hugging Face, Apache-2.0 | the pretraining mixture (FineWeb-Edu, DCLM, The Stack, curated math and code) and the instruction data are published | huggingface.co/HuggingFaceTB/SmolLM2-1.7B-Instruct |
| RedPajama-INCITE Chat 3B v1 (q4f16, 1.5 GB) | Together, Apache-2.0 | RedPajama-1T, the open reproduction of the LLaMA data; OASST1 and Dolly 2.0 for instructions; window 2048 | huggingface.co/togethercomputer/RedPajama-INCITE-Chat-3B-v1 |

Llama 3.2 3B, the rung's original single model, is no longer offered: its training data is undisclosed. The cards are cited here and not in `webllm-rung.js`, because that module is loaded by the page and the constitution's host scan (II.13) keeps every non-local address out of anything the page can reach — the rung's own test caught the first cut carrying them.

**Same-origin, mirrored, verified.** `models/fetch-webllm.sh <id>` mirrors one roster model from the address the VENDORED library's own catalog entry names (never typed in the script), resumable, every shard md5-verified against the manifest the engine loads by (`tensor-cache.json`; OLMo 2 ships only that one), every tokenizer file the model's config names (OLMo 2 and SmolLM2 name `vocab.json` and `merges.txt`; the first cut missed both and the rung test's shard walk refused the half mirror). A localhost page loads every byte from `models/`; a static deployment serves that same directory as the site's own bytes — the whole point of "people can run it from a website": the site is the origin, WebGPU is the runtime, nothing leaves. `models/*/` stays gitignored; a checkout without a mirror skips the mirror walks typed, a mirror that drifted from the installed engine fails them.
...
**The ask, and the question under it.** "Have it try public proxies and learn what are open gateways" — and then whether those gateways could stand in front of the maintainer's own n8n proxy so the maintainer "can't tell who is requesting what site." The answer, which this entry records as the decision: a relay in front of a proxy hides WHO only if the relay strips the person's address (measurable, per relay, below), and never hides WHAT — every target address still lands in the proxy's own execution log, with a time, and at small numbers what-and-when identifies a person about as well as an address does. The design that has the wanted property is Oblivious HTTP (a relay that sees who but not what, a gateway that sees what but not who), which is more than a webhook. So the maintainer's proxy stays out of the loop, as the launch pass already made it: direct fetch, then public gateways, every route disclosed to the person and recorded on the person's own machine. Nothing about a person reaches the maintainer.

**What shipped.** `native/organs/web.js` (the web organ's pure half, where the archive addresses already lived) names a roster of six public gateways — the Wayback Machine (availability API, then the snapshot's raw face), r.jina.ai (a reader), api.allorigins.win, corsproxy.io and api.codetabs.com (relays), archive.today (an archive) — each with its kind and a plain sentence of what it sees; `blockedShape` types the refusal a gateway can get around; `readGatewayBody` reads each gateway's answer back into a page; `foldGateways` folds the record's own `web-gateway` and `web-gateway-leak` lines into counts, and `rankGateways` orders the roster by natural frequency of answering — an untried gateway before one that only failed, the faster among equals — without a threshold and without pruning: every gateway stays tryable, the record says what happened last time. `explore-server.mjs::fetchAndKeep` tries them in that order when the direct fetch is refused, marks the history entry `via` with the gateway, its kind, why the direct fetch was refused and what the gateway saw, and records every try; `GET /api/web/gateways` is the learned table (no egress); `POST /api/web/gateways` probes each gateway with one canonical page AND, for each relay or reader, fetches an echo endpoint through it to read back the headers it forwarded — the person's own address, read directly off the same echo, decides `forwardsAddress`. The chat's `/gateways` door shows the table; `/gateways probe [url]` runs the probe; a fetched page's line in THINKING says "via <gateway> (the direct fetch was <why>; <what it sees>)". `native/tests/web-gateways.test.js` 5/5, offline.

**Measured, 2026-09-05, from this machine.** Probe: of six gateways ONE answered — the Wayback Machine (example.com's snapshot, 559 chars, ~2.7–5.9 s); r.jina.ai and corsproxy.io now demand a key (401); api.allorigins.win and api.codetabs.com are down (522/503); archive.today rate-limits (429). bloomberg.com refused the direct fetch (403, "Are you a robot?"): all six were tried in the learned order and none answered — the Wayback snapshot of that homepage exceeds the 8 MB byte cap, a typed refusal on the record, not a silent one. reddit.com answered 200 with a title and an empty text face (a script shell): the empty-face shape routed it through the Wayback Machine, whose snapshot is the same shell — the route is disclosed, the zero is honest. The leak probe could decide nothing today, because no relay or reader was open to echo through; the table says "not measurable while closed" rather than "safe". This is the point of learning instead of asserting: a roster written a month ago would have promised five relays that do not exist this afternoon.

**What is not claimed.** That any gateway is open tomorrow; that a gateway which does not forward an address today will not; that a snapshot is the live page (the entry names it as a snapshot, with the archive's timestamp when it has one). The gateways are third parties the person reaches by choice (the web toggle) and only after a refusal; README's "what leaves" paragraph says so.

## P118 — Three homes, one page: a terminal, a static site, a browser extension; the build is root-independent, the weights climb a ladder, and the page probes its routes at boot and says what it found (2026-09-05)

**Generality:** universal for the rules (the page's static build rewrites only server mount-point specifiers, into relative paths from each file's own place, so the same build runs at a domain root, under a subpath, from an archive.org item and from a chrome-extension:// origin; a rung's weights are sought at the site's own `models/`, then at every mirror the site names, then at the publisher — the first base whose config answers is taken and stated; what the page can reach is probed at boot, never assumed from where it is, and said once on the status chip and in full at `/routes`); specimen-scoped for the hosts measured.

**The ask.** "We'll be on GitHub sites, but also started via terminal or maybe a Chrome extension — so have it try the various routes as needed." And before it: can archive.org pin it permanently — yes for the bytes (an item holding the build with its checksums; the weights are ~3.3 GB of the 3.6), no for running from the item until the page stopped depending on server mount points, which this pass ends.

**What was wrong.** The page reached its engine through root-absolute specifiers a server maps at request time (`/engine-v7/`, `/engine/`, `/nul/`, `/node_modules/`, `/priors-data/`), so any static host died at import time; the Pages workflow sed'ed those into one hardcoded subpath and still checked out the old engine repo, so the deployed site was broken twice over; the in-tab rung read its own `models/` only on localhost and went to the publisher from every other origin, so a pinned copy could never be self-contained; the WebLLM client and worker had shipped unwired (P116) and, wired, assumed one home.

**What shipped.** `deploy/build-site.mjs` builds `dist/` as the siblings the repos already are (`the-fold/`, `eoreader7/`, `node_modules/`), derived from `page-graph.mjs`'s walk of what index.html loads (87 first-party files, three mounts, seven vendored packages), rewriting exactly the five mount prefixes to relative paths per file depth and nothing else — `/api/*` stays absolute and fails visibly where there is no server; `--models link|copy`, `--mirror <base>` (written to `models/MIRRORS.json`), `--extension` (an MV3 `manifest.json` with `'wasm-unsafe-eval'` and host permissions for the local servers and the two weights hosts, plus a one-line `background.js`), `SHA256SUMS` and `BUILD.json` (commit, files, bytes). `.github/workflows/deploy-pages.yml` now checks out eoreader7 with its submodule beside the-fold, runs the build's own assay, then the build. `webllm-rung.js::weightsBases` is the ladder and `webllm-client.js::chooseWeights` walks it once per page with one small GET per base, taking the first `mlc-chat-config.json` that answers; the status line says which. `routes.js` reads the home off the page's own address (`whereAmI`) and phrases what the boot probes found (`describeRoutes`): Ollama (what the picker found), WebGPU (the rung's blocker), the explore server (one GET of its lightest route), serve.mjs's API (reachable exactly when the page's own origin serves `serve.mjs` — a static home never does); `/routes` re-probes. Tests: `build-site.test.mjs` (a real `--no-vendor` build into a temp dir: every page file, no mount specifier left, every mount tree beside it, the checksums cover every file, the server is not in the page), `routes.test.mjs`, the ladder's cases in `webllm-rung.test.mjs` — 30/30.

**Measured, 2026-09-05.** The full build: 7,521 files, 455 MB on disk without weights (3.8 GB with the three models linked in). Served by a plain static host with no aliases and no APIs — what GitHub Pages is — at a subpath: the page loaded every one of its 171 resources from its own origin, found Ollama on localhost:11434 and the explore server on :8812 beside it (the routes discipline: a home never decides a route), reported `/run off` because no origin served serve.mjs, offered all seven rungs, and OLMo 2 1B answered in-tab with its weights read from the site's own `models/` — the first rung of the ladder. `/routes` printed the table. The extension package is built and its manifest assayed, not installed: this pane cannot load an extension. Pages itself was not deployed from here; the workflow is the same build.

**What is not claimed.** That the archive.org item exists (the pin is a `--models copy` build plus the user's own `ia upload`); that a Pages visitor without Ollama or WebGPU has a mouth (the report says NO MOUTH in so many words); that the extension has been run.

## P119 — The room: a chat preserved, shared, and answered through a Matrix homeserver the person names, with nothing readable ever leaving the page — and a pool of the members' machines as mouths (2026-09-05)

**Generality:** universal for the rules (everything a homeserver receives is either a pointer, a public key, a key wrapped to one member's public key, or ciphertext under a chat key that lives in no event; the chat key travels only wrapped to a member or in a share link's fragment; every record line passes the leak instrument first; a member's mouth is a rung like any other and a prompt to it is sealed the same way; no homeserver is named or preferred by the page; every member of a room holds full power); specimen-scoped for the numbers measured against the adversarial fixture and in the browser pane.

**The ask.** "Add our style of Matrix integration so people can preserve history and also share chats with others. All data encrypted, including data uploaded to the Matrix media store (triple check it's actually encrypted, do tests that prove it)." Then: "give no priority to any Matrix account — I mean like matrix.org"; "let this let users use other machines to run inference and, if possible, pool resources"; "don't flatten room power, give full powers"; "make a surface to see pooled devices and resources."

**Our style is eopm's** (its ENCRYPTION-DESIGN.md, crypto/envelope.js, blockcodec.js, blocks.js, invitelink.js), carried over rather than re-invented: the room is a place to put opaque bytes and to hand out the one key that opens them. Not Megolm — device-scoped and forward-secret, the wrong shape for a record every authorised reader must read in full, forever, from a fresh browser — and not Matrix's encrypted-attachment format, whose per-file key would sit in cleartext event content without Megolm.

**What the homeserver holds.** Media store: blocks, AES-256-GCM under the chat key ([iv12][ct+tag]), hash-linked (each block names the previous block's mxc and the SHA-256 of its ciphertext). State: `fold.chat` (ours, no content), `fold.member_key` (each member's ECDH P-256 public key), `fold.chat_key` (the chat key ECIES-wrapped to the slot's member — ephemeral pair, HKDF, AES-GCM — plus `grants` wrapped to other members' current public keys, on the granter's own slot because Matrix lets only a user write a state event keyed by their own id), `fold.chain` (one member's head + manifest of pointers, capped to the state-event budget), `fold.mouth` (the models a member offers, and from which home). Timeline: `fold.job` (an address, an id, a seal, a size) and `fold.answer` (the job, a seal) — a seal above 48 KiB rides the media store as ciphertext with its hash. So an operator can read: who is in the room, who invited whom, the room's name, how many blocks of what size went up when, public keys, who offers which model names, that a sealed job went to whom and when its sealed answer came back. Never a turn, a prompt, an answer, a source.
...
**Found on the way.** In the Chromium 148 of the browser pane a `<dialog>` fires `toggle` but never `close`: the sign-in sheet's first cut, and the page's own paste sheet before it, did nothing on close there. Both now act on the form's `submit`, with a flag against a browser that fires both. The pane's synthetic Return and Tab keys never reach a dialog's fields either — a fact about the tool, not the page; the sheet was driven by its button.

**What is not claimed.** A real homeserver was not used from here: the proofs are against the adversary, which implements the routes the crossing uses and the auth rules the design leans on; the first sign-in to a real one is the person's step, and the page names none. Identity is per browser: a wiped browser reads again only from the link or after a member's next `/share` grants its new key (eopm backs identity under an account key; this does not, yet). Sources are not preserved — the room holds turns; the door says so. A member who serves in-tab models serves them at that page's pace. The room's name is the one typed thing the server holds. Session, identity pair and chat keys are at rest in localStorage, like the reading record, unencrypted.

## P120 — A share link bound to one account: no key in the link, a proof only that account can publish, and the granter wraps to that key alone; plus epochs, passphrase links, and a vault at rest (2026-09-05)

**Generality:** universal for the rules (a link that names an account carries no key at all — it carries a one-shot secret, and the key is handed over only after that account publishes a public key whose HMAC proof over room, account and key verifies against the secret, so neither a forwarded link nor a homeserver that substitutes a key obtains anything; a link that does carry the key says in plain words that it is a magic key; every block names the key epoch that opens it, so a new epoch excludes without rewriting history; what a browser keeps at rest can be sealed under a passphrase, and every door is a typed gap until it is unlocked); specimen-scoped for the timings and counts measured against the adversarial fixture and in the browser pane.

**The ask.** "Key is like a magic key, tell users to be careful with it. BUT let them make it specific to a Matrix user so it only works if it verifies they are with the account using the auth key for that account." And: "make those updates we discussed" — the five follow-ups named in the session's security answer. The stated worry is the homeserver operator reading what is stored, including the media store, rather than a nation-state adversary.

**What was wrong.** P119 had one kind of link, and it carried the chat key. Whoever held it read the room, forever, with no revocation; a member's re-published key was granted on sight, so a homeserver that substituted its own key would have been wrapped to; there was one key per room for all time; and the session, identity pair and chat keys sat in localStorage in the clear.

**What shipped.** Three kinds of link, one shape (`buildShareLink`/`parseShareLink`, v2; v1 links still read as open). **Bound** — the default whenever someone is named: the room, that account's Matrix id, a 128-bit one-shot secret and a 7-day expiry, and NO key. The recipient must be signed in as that account (a mismatch is refused by name), joins, and publishes `fold.member_key` carrying `proof = HMAC-SHA256(secret, "fold-invite\nroom\nuser\npub")` — a state event Matrix lets only that account write. A member holding the secret (`grantPending`, run by `/share`, by the page's `watchInvites` while it is open, and by `matrix-worker.mjs` on every sync pass) verifies the proof against an unspent, unexpired invite and wraps the key to THAT public key; anything else is refused with a reason. **Open** — the magic key, printed with `MAGIC_KEY_WARNING` every time. **Passphrase** — the key sealed under three or more words said aloud; the link alone and the words alone open nothing. A key with no proof is never granted automatically: it is listed as unverified with a 16-hex-digit fingerprint of its public key, and `/share grant @who` wraps to it after the two people compare fingerprints out of band (`/matrix fingerprint`, `/matrix members`). **Epochs**: every block, manifest entry and chain head names its epoch; `rotate` mints a new key and re-wraps every epoch to every currently covered member, carrying older epochs in `older` so a new reader still opens the whole record; `remove` kicks and rotates excluding them. **The vault**: `/matrix lock` seals everything this browser keeps under PBKDF2-SHA256 at 600,000 rounds; storage then holds `{v, vault:{salt, rounds, blob}}` and nothing else, every door answers "locked" until `/matrix unlock`, and `/matrix unlock off` returns to plain storage on the passphrase.

**The proofs** (`matrix.test.mjs` 13, `matrix-client.test.mjs` 21, all against the adversary that keeps every byte): a bound link's payload carries no key in any encoding; mallory holding it is refused by name; dave redeems it and reads the whole record; a second use is refused as spent; **an operator that substitutes its own key under the member's state key gets nothing — the proof no longer verifies and the grant is refused, and putting the real key back lets the grant land**; an expired link is refused by both sides; the wrong words are a typed gap; after removal the excluded member's browser reports a typed gap naming the epoch it holds no key for, while a re-granted member reads both epochs; the vault leaks nothing to storage and refuses the wrong passphrase; a re-published key holds no wrap until it is granted again. The three P119 checks (bytes, structure, function) still hold on the new shapes.

**Measured in the browser pane, 2026-09-05.** Alice signed in, preserved, and ran `/share @bob:fake.test`; the printed link's payload held the account, a secret and an expiry, no key. Bob's page, at another origin, read the link, said the chat was for bob, and prefilled his name; on sign-in it published his key with the proof, alice's watcher granted it within seconds, and the two turns came back decrypted — both panes naming the same fingerprint, 46e7-fd86-3dec-3e77. `/matrix members` showed alice's key unproved (she made the room) and bob's proved by a bound link. Then `/matrix remove @bob:fake.test`: rotated to epoch 1, bob's next `/matrix open` answered "not a member", and alice's next `/preserve` sealed block 1 under epoch 1. Over the whole rehearsal the fixture saw 72 requests; the leak instrument found the two login passwords and nothing else — not the invite secret, not a key, not a turn, on the wire or on its disk.

**What is not claimed.** A leaked *open* link is still absolute: the warning is the only defence, which is why a named share is now the default. Rotation does not un-read what a member already read. The bound proof binds a key to an account and a link, not to a human; an account taken over redeems its own link. Fingerprints must actually be compared out of band to be worth anything. The vault protects storage at rest, not a page already unlocked in a tab. No real homeserver was used from here.
...
**What it does not do.** It does not raise the binding rate of the relation tier; it places every sentence honestly on what did reach it. Where the highest rung is the model's name, that is the finding.
...
**Files.** `piece-revise.js` (`revisePiece`, `revisionLine`) + `piece-revise.test.mjs` (the ungrounded rewrite refused; the control: nothing later, nothing revised, no ask spent); `holon.js` (the revision pass after the editor for a piece — the whole piece's passages as one pool for the reader, the ledger's notes and disputes, the derived facts, the cast; claims anchored to their sentences by first end and label; `revisions` on the result); `app.js` (the piece carries its model; revisions on the ticker and on `longform-done`). `holon.test.mjs` 70.

**Amendment to P117 — dice, run 5.** Same ask, gemma2:2b: `counts_face` now returned real face counts `{6: 18, 4: 12, 2: 24, 3: 19, 1: 13, 5: 14}` (witnessed by the run and handed to the next snip); `prints_histogram` imported a package the sandbox does not carry and the one fix ask did not undo it; final run failed; the model wrote 655 of 1,144 chars (57%), 0 refused, 1 fix. The next mechanical rung is named, not built: an import the sandbox refuses is a NameError's sibling — the run's own message names the module — and the repair is the same shape (strip or qualify), the runtime deciding.

**Amended same day — run 8, on the anchored ladder and the revision pass.** 16 sections, **13,715 words** (27.4 pages of 30), every section between 468 and 746 words, 0 unsupported, 51 unbacked, 26 minutes; the revision pass re-cited 4 sentences and rewrote none (no later reading denied an earlier claim); 3 re-asks, 1 meta cut, 1 hunt. **The ladder's tally is the finding:** of 463 sentences, 1 bound, 6 witnessed, 38 with names established and the claim not, **418 the model's own**. The anchoring fix worked (run 7 had 0 bound because the claim never knew its sentence) and the rate barely moved — so the wall is not the ladder. It is two things this instrument already knows by name: the relation tier binds only an exact arrangement (the paraphrase wall, P74/P86), and a 2B mouth writing 13,000 words from two pages writes most of them past its material. The witness, asked 24 times a section, said "states" six times in 463 — the select protocol is strict by design (P83's own finding on Dracula: one in sixty), and here it is the honest count. What would move the number is upstream of citation: more material in scope per section, and a witness at the paraphrase grain; neither is a citation lever. One residue found in the addresses: a `holon.js` chunk reached a section's pool although code sources are out of scope — the pool a part retrieves over is not the scoped `live` everywhere; named for the next pass.

**Amended same day — the scope leak found by run 8's addresses, closed.** The named-source block at the top of the turn ends by re-reading the whole material pool (a page the ask names must join it), and it runs whenever web consent is on — so a piece's scope, applied a few lines earlier, was undone before the plan: seven `holon.js` passages and two of the plan of record were retrieved for sections about a television series (run 8's own record: 53 retrieved, 33 from the fetched page, 9 from a second page, 7 from `holon.js`). The scope is now re-applied after that block, with any page this ask itself named admitted by construction. Run 9 is the measurement.

**Amended same day — run 9, the scope fix measured.** Retrieved: 32 passages of the fetched article, 9 of a second page, 2 of the search digest — and nothing else (run 8: seven of `holon.js`, two of the plan of record). 15 sections, 12,207 words, obligation coverage **0.71** (run 8: 0.58 — the cast the sections were told to cover is now the material's own), 2 re-cites, 1 restated sentence cut, 0 unsupported, 66 unbacked, 23 minutes. The ladder: 1 bound, 3 witnessed, **78 names established** (run 8: 38 — clean passages resolve more names), 325 the model's own of 407. The scope fix moved what it could move — coverage and the names — and not what it cannot: the binding rate stays at the paraphrase wall.

## P121 — A piece in two faces: markdown that reads, with footnotes of verbatim spans and text-fragment links; html with real anchors and a json sidecar (2026-09-05)

**Generality:** universal for the rules (a footnote is the source's own bytes at the span — sliced from the passage by address, never retyped — and a link that opens the page AT the span, the browser's own text fragment; a sentence nothing read places wears one shared footnote that names the model, a sentence the witness refused another; the html carries each sentence as a span with its tier, its cell, its address and its byte offsets as data, the same links, no script and no remote resource; the json sidecar is the same structure as data, prompts included; the three land under `record/pieces/` as files a reader opens, the landing a record event). User direction: "two types of output, one that uses those Google hyperlinks that cite specific spans and then footnotes with mechanically produced verbatim spans, that way the MD reads fine; and an html that uses real anchoring and a json sidecar."

**Files.** `piece-export.js` (`textFragment`, `verbatimSpans`, `exportPiece`) + `piece-export.test.mjs` (3: fragment forms; spans sliced by address for bound / witnessed / named; the two faces and the sidecar); `app.js` (`state.lastPiece` kept at a piece's end — sections, urls by source, prompts by section, the ladder's context — `exportLastPiece` placing every sentence again and handing the faces to the explore server, links under the message, `/export` for the last piece again; hunted pages carry their source name so an address maps to its url); `explore-server.mjs` (`POST /api/piece-export` → `record/pieces/<slug>-<stamp>.{md,html,json}`, a `piece-export` record event). Counts from the first exported run go in the amendment.

**Amended same day — the html snips the source on click (user direction: "the html should let me click and see the source content snipped").** The passages a piece cites ride inside the html as `<template>`s; a click or Enter on any sentence opens a panel with the cited passage's own bytes, the span marked, its address and the link that opens the page at the span; a sentence nothing read places says so by the model's name. Plain inline script, no library, no host. The json sidecar carries the cited passages' text too, so the html can be rebuilt from it. Verified live on the fixture page: the bound sentence opens its passage with "The X-Files is an American science fiction drama television series created by Chris Carter." marked; the witnessed one with "It premiered in 1993."; the model's own with the model's name. Two things the first build got wrong and the click found: a template's text lives in its content fragment, not its textContent; and the first export's "names established" notes carried "Some", "Trust", "Want", "Believe" — a lone sentence-initial or quoted-title capital read as a name (L2), refused by structure now.

## P122 — The section checks its own atoms against the snips it stood on, in process: the cheap rung first, the ask only where a flag stands, the rewrite landed only when it clears (2026-09-05)

**Generality:** universal for the rules (a section is handed the verbatim, addressed sentences of its own passages that carry its obligations and its topic — the snips — above the rest of the material; every number, date and name it then writes is looked for in a snip beside a word of the sentence's own, P31's company rule, with no model; a flag names what was looked for and the absence it stands in; a snip sharing the sentence's words and carrying a different year is a contradiction candidate that names the source's year at its address; one bounded rewrite ask per section carries the flags as facts and the snips as the only ground; a rewritten line lands only when its own atoms clear the same check and it shares a word with a snip — the mouth's correction is never trusted, it is checked again; a flag the rewrite could not clear stays on the record, and the witness is spent on that sentence before any other). Local for the numbers (`SNIP_MAX = 40` snips a section is handed; the sentence grain of a snip; the two-content-word overlap that makes a contradiction candidate — all declared, none measured). The user's question: "I think the essay has a few mistakes. now that we have the snips, what is an efficient way for it to evaluate the accuracy of its claims in process?" — and the answer built: check the cheap thing first, spend the ask where a flag exists, revise while the section is still in hand.

**What a cut is not.** A contradiction candidate here is the MOUTH's sentence against a SOURCE's snip. It is not landed as a cut on the ledger: a cut is a source's denial of a link on the record, and the mouth's sentence is not on the record — `self:model` may never dispute (P2). So the candidate is a flag on the sentence with the source's year and address beside it; the rewrite is asked to say what the source says; a refused rewrite leaves the sentence flagged in the export's check line. Two SOURCES disagreeing on a year is the reader's contest (P104), not this organ's.

**The control (built to fail, `snip-check.test.mjs` and `holon.test.mjs`).** The same draft against the snips of an unrelated passage flags more; a rewrite whose year is still wrong is refused and the original stands, flagged; a rewrite sharing no word with any snip is refused whatever its atoms; a one-letter needle hands no snip; a preamble line ("Here are the rewritten sentences:") is not a sentence and shifts nothing.

**Files.** `snip-check.js` (`snipsFor`, `snipBlock`, `atomsOf`, `checkSentence`, `checkSection`, `reviseAsk`, `applyRewrite`, `SNIP_MAX`) + `snip-check.test.mjs` (3); `holon.js` (the snips built beside the obligations from the prose passages, the block above the material, the check after the meta cut, the one rewrite ask, the witness ordered flagged-first, `snipCheck` on the section's `piece` result) + `holon.test.mjs` (P122 + control); `app.js` (`longform-done` carries the totals — snips, atoms, supported, flagged before and after, asks, outcomes, contradictions; `exportLastPiece` hands each section's check to the export); `piece-export.js` (a check line under each section in md and html, the object in the sidecar) + `piece-export.test.mjs`. Suite 1,636/0. Live measurement: below, as amended.

## P123 — The thinking-depth slider: deeper is more passes over the same bounded material, never more context (2026-09-05)

**Generality:** universal for the rule (a rung of the slider is a set of recursion budgets — how many times a section is read against its snips and rewritten, how many sentences are put to the witness, how many times a denied sentence is revised, how many continuations a short draft gets, how many hunts a thin retrieval gets, how many cited links are opened; level 1 IS today's declared budgets, so the base layer is a fine response alone and every rung above is additive; asks double per rung, rounds add one; level 0 runs every mechanical check and spends nothing on recursion; an explicit caller value wins over the rung; the model's window is identical at every depth). Local for the numbers (declared in `depth.js::budgetsFor` over `holon.js`'s own constants; none measured). User direction: "the output becomes better the same way people make better output, not by expanding their context window but by … so the way we switch models and reasoning, this can toggle deeper thinking through a slider."

**Why this shape.** A person's better draft is a second reading, a colleague's eye, one more pass — recursion over a bounded working memory — not a bigger memory. The instrument already had every pass as a declared, bounded act (P110 re-asks, P115 witness, P116 revision, P122 snip rewrite, P108 continuation, P114 hunt); the slider only says how many of each. So the slider sits beside the model picker: the same kind of choice, disclosed in the status line (`ready · gemma2:2b · depth 2`), on the record (`longform-done.depth`) and in the export's key line (`depthLine`).

**Files.** `depth.js` (`budgetsFor`, `depthLine`, `DEPTH_MAX`, `DEPTH_NAMES`) + `depth.test.mjs` (3: level 1 = base; monotone, 0 = nothing, doubling and +1; the legend in plain words with no canon); `holon.js` (`depthBudgets`, `depth` on `runHolonicTask`, `pieceWitnessAsks`/`snipRounds`/`continuations`/`hunts`/`linkBudget` on `runPart`, the hunt in rounds with a different query each, the continuation in rounds, the snip rewrite in rounds where a refused round earns another and only a repeated reply ends it, the revision pass with the rung's rounds and asks, `depth`/`budgets`/`depthLine` on the result) + `holon.test.mjs` (P123: depth 0 asks nothing and hands the witness 0; depth 2 asks a second rewrite after a refused first and lands it); `index.html` (the slider in the model menu with its legend); `app.js` (`state.depth` from `fold-depth`, `readyLine`, `renderDepth`, `depth` on the turn call, the record and the export). Suite green. Unmeasured: the same ask at two rungs — the long-stream harness takes `--depth` for exactly that (P124).

## P124 — The long-stream stress: six large files of six kinds, N turns through the real turn, a probe every fifth turn scored with no model (2026-09-05)

**Generality:** universal for the rules (the fact bank is read off the material with no model — a sentence with two whole-token atoms and a name among them, sampled evenly across each source's extent; the four probes are built from it — a cloze the material can fill, a "what did you answer N turns ago" whose ground is the transcript's own atoms, a false premise made by moving one atom, and two sources' exact difference — and each is scored by whether an atom appears, never by a model's opinion; the ledger and grid are threaded turn to turn and the recency window is the app's own, so memory past the window can come only from the record; a failed turn is recorded and does not advance the transcript; the run is resumable from its own state and reproducible from its seed; the configuration is printed before the first turn, P88). Local for the numbers (probe every 5, 60 facts per source, memory distances 5/20/50/100/200/500, 1,500-char windows for a paragraph-less file — declared). User direction: "load up multiple very large different types of files and run chat for 1,000 turns with adversarial tests on its recall, memory, and reasoning."

**What is not claimed.** A probe's `hit` is containment with company; it is not truth (the material can be wrong). `evaded` on an injection is an honest refusal and is counted apart from `capitulated`. A memory probe's ground is what the mouth SAID, not what was true. Pace and outcome numbers are dated results no test reads (P94).

**Files.** `eoreader7/native/eval/the-fold/lib/long-stream.mjs` (`buildFactBank`, `makeRng`, the four probes and their scorers, `organicQuestion`, `scheduleFor`, `memoryDistanceFor`) + `native/tests/long-stream.test.js` (6, with the control: an unrelated answer is not a hit); `eval/the-fold/long-stream.mjs` (the driver on `lib/product-assay.mjs::organs`), `long-stream-score.mjs` (summary.md/json), `LONG-STREAM-RUNBOOK.md` (launch, watch, resume, score, the edit loop). Validation runs and the first live 1,000-turn numbers: amendments below.

## P125 — A wrong answer is corrected at any level, and the premise a question smuggles in is checked before the mouth sees it (2026-09-05)

**Generality:** universal for the rules (the atoms-against-snips check and its gated rewrite run for ANY turn with material, not only a piece's section — same company rule, same gate, a rewrite landing only when its own atoms clear the check; a claim the question states as ALREADY ESTABLISHED has its atoms looked for in the same material before the model drafts, and what the sources actually say is handed in as a fact with its address; a rewrite must keep the subject of the sentence it replaces). Local for the numbers (the rounds are the depth slider's rung, P123). User direction: "wire it up so wrong answers get corrected at any level."

**The measured failure this closes.** Long-stream run 1 (S77), turn 15: the probe took a real constant from POLICIES.md and swapped one token of its name, then asserted it as established. gemma2:2b answered "You're right, we established that `EFFECT_READS_THE_Sherman_RUN` is the named export that states it" and confabulated a meaning for a token the material never contains — 100% capitulation across the run's injection probes. Nothing had checked the QUESTION, and nothing corrected a plain turn: P122's rewrite was piece-only. The fix is mechanical on both sides and asks the model for no posture at all (the model is just the mouth): the premise's atoms are looked for, and the source's own sentence goes in as content.

**A correction is about the same thing.** Measured in run 2: asked to fix "The sources describe Prince Andrew's meeting with the Emperor", the mouth returned "Our order should provide means to that end" — atoms that happened to sit in some snip, so the gate let a non sequitur land in place of a claim. A candidate must now share a content word or an atom with the sentence it replaces, or the original stands flagged.

**Caught in this project's own fixture.** The completeness-gate test's draft asserts "Lincoln appointed Hamlin in 1861"; `LINCOLN_TEXT` contains no year at all. The gate never saw it because a gate over fillers does not read atoms. The pin now counts the two asks apart and reads the flag.

**Files.** `correction.js` (`premisesOf`, `checkPremises`, `premiseFacts`, `correctTurn`, `turnSnipBlock`) + `correction.test.mjs` (5, with the control: a premise the material does establish produces no facts block, and no material is a no-op); `snip-check.js` (the same-subject rule) + its pin; `holon.js` (the premise block and the snips above the material for any turn, the correction pass after the draft, `correction`/`premises` hoisted onto the turn) + `holon.test.mjs` (P125 end to end, and the fixture's fabricated year). Suite 1,655/0.

## P126 — What was wrong once is kept, and handed back: corrections as entries in the room's own permanent chain (2026-09-05)

**Generality:** universal for the rules (a correction is an entry in the shape the chat's chain already carries — `kind: "correction"` beside `kind: "turn"` — so it is sealed, hash-linked and permanent by the same machinery, deduped by CONTENT identity so two machines converge without knowing of each other, and merged across members by `mergeChains`; the corrections in scope for a question are handed back as facts before the next draft; scope is a shared atom, or two shared content words). Local for the numbers (`RECALL_MAX = 4`, `SCOPE_WORDS = 2` — declared, unmeasured). User direction: "we don't need a system that's always right, but we do need one that is actively learning to get better … see how we now can have permanent memory for an instance."

**Why the room and not a new store.** P122 (the room, PR #143) already gives an append-only, AES-256-GCM-sealed, hash-linked record readable from a fresh browser forever and shared with every member. A correction is exactly that shape, so it needs no second mechanism, and a correction learned on one machine reaches every other member's mouth. The instrument does not learn by growing its context; it learns by carrying forward the small, addressed record of what the material actually said. `learned.js` stays pure and imports no crypto: it emits the entries `matrix.js` seals, and the two meet only at their shared shape.

**THE LEARNABILITY WALL — the dangerous failure, measured live within nine turns.** The first learning run stored 72 "corrections", of which 63 were poison: 60 markdown scaffolding ("* **What is the trigger's claim?**"), one echo of the instrument's own correction prompt, and — the one that matters — **two honest refusals**. "The provided text snippets don't contain a passage about Scheria" was recorded as a wrong answer. A system that learns not to say the material is silent is a system learning to FABRICATE. The null never convicts (THE-NULL-STATES, law 4): a stated absence is a finding, never an error, and is never learnable. Nor is a refused rewrite learned — the instrument knows the answer is unplaced, not what the right one is.

**Files.** `learned.js` (`correctionEntry`, `learn`, `recallFor`, `learnedFacts`, `fromOutcomes`, `fromPremises`, `learnable`, `correctionsIn`) + `learned.test.mjs` (6, with the control: an unrelated correction is never recalled, and every poison shape from the live store is refused by name); `holon.js` (`learnedStore` down to every part, `learned`/`learnedUsed` out on the turn); `eoreader7/native/eval/the-fold/long-stream.mjs` (the store kept beside the runs, not inside one, so it is permanent across them; appended the moment a turn learns, so the very next turn is handed it).

**Amendment to P126, the same day — the learning went the wrong way, and was measured doing it.** The first design handed the corrections in scope to the model as a block naming the error in order to forbid it ("… was said here before and nothing read supports it"). Run twice against the same injection question, the same material and the same seed:

| | answer |
|---|---|
| without the store | "That passage doesn't exist. It's not in the sources I have access to." |
| with the store, naming the error | capitulated, and explained the thing at length |

Repeating a falsehood in order to negate it hands a small model the falsehood; the negation is the part it drops. So the block now carries ONLY the positive sentence the sources actually hold (`learnedFacts` filters to entries with a `corrected` value and never prints the claim it replaces), and the negative half — a claim known to be unplaced, with no positive to put in its place — never reaches the mouth at all. It is kept by the instrument as a guard (`learnedGuard`) and spent on the DRAFT: a sentence that repeats a known-false claim (sharing an atom and a frame word with it) is cut before the answer ships (`repeatsKnownFalse`, `repeatedKnownFalse` on the turn). With the fix, the answer with the store is identical to the answer without it, and the guard still catches a repeat. **The general rule this instrument now holds: negative knowledge belongs to the checking, never to the prompt.** Suite 1,658/0.

**Amendment to P124 — a stated absence is not a dodge.** The injection scorer filed "That passage doesn't exist. It's not in the sources I have access to" as `evaded`, beside answers that said nothing either way. They are different acts (THE-NULL-STATES, law 3: a null is a finding when the search reached its object), so `refused` is now its own verdict and `capitulated` is the only failure among them.

**Amendment to P125 — the premise check was silently missing every decomposed turn.** `runPart`'s `question` is the PART's words (`<label> <description>`), not the person's. A claim asserted in the question therefore appeared in no part's question at all, `premisesOf` found nothing, and the check never fired — measured live (S77 run 4, turn 15: an injection reached the mouth with `premises: null` and was not refused). A premise is a property of the whole turn, asserted once before any plan existed, so it is now checked against the TASK, exactly as `searchedVoid` and `answerShape` already are for the same reason. The learned recall reads the task's words too. Pinned by a decomposed-turn test. Suite 1,659/0.

**Amendment to P125, the same day — the rule was written and then missed one file over.** P126's finding (negative knowledge belongs to the checking, never to the prompt) was applied to `learnedFacts` and NOT to `premiseFacts`, which went on quoting the question's false claim back at the mouth in order to deny it. Measured live (S77 run 5, turn 15): the premise check fired correctly (`unverified: 1`, the value "Durham" in no passage), the block said so — and the mouth explained the passage anyway, inventing a "Durham investigation" to make sense of it. **A check that only advises is not a check.** Two changes: `premiseFacts` now carries only positives (the source's own sentence where it speaks of the same thing, or one line naming ONLY the absent value, never the claim), and `premiseGuard`/`repeatsAbsentPremise` keep the absent values for the instrument, so a drafted sentence asserting the token the sources lack is cut before the answer ships. Verified against the exact live case: the capitulating sentence no longer ships, and an honest sentence about the same subject survives. Suite 1,660/0.

## P127 — The mouth narrating its own answering is cut from any grounded turn, and an address is never mistaken for an answer (2026-09-05)

**Generality:** universal for the rules (a sentence that narrates the act of answering — a heading, "Let me break down…", "This analysis focuses on a passage from…" — and says nothing whatever about the material is cut from any turn that HAS material; a passage-less turn is conversation and is left alone; the cut never touches a stated absence, a sentence carrying a name, number or date, or one that uses a word the material itself uses; an answer that is nothing BUT narration is left whole for the marks to carry rather than shipped blank). Local for nothing measured.

**Why a second cutter beside `cutMetaTalk`.** `cutMetaTalk` (P110) matches the PIECE's own instruction vocabulary, so it is blind to this: none of "analysis", "break down", "focuses", "purpose" appear in the template. Measured through the long-stream run, answer after answer opened `## Identify the passage` / "This analysis focuses on a passage from the `holon.js` file" and then discussed the act of answering. The narrow construction is the point — the two things it must never touch are the two that matter most: a stated absence is a FINDING (THE-NULL-STATES, law 3), and any sentence carrying content is answering whatever it sounds like.

**And the scorer: an address is not an answer.** `long-stream.mjs`'s recall check looked for the expected number anywhere in the answer, and a cited span (`name#12345-67890`) is full of digits — a probe could be credited with a hit the mouth never made, or a miss recorded as "wrong" on a number that was only ever an address. Addresses are stripped before any number is looked for, and before listing what the answer stated. Names are unaffected. Pinned both ways.

**Files.** `correction.js` (`cutProcessTalk`) + `correction.test.mjs`; `holon.js` (the cut on any turn with passages, `metaCut` surfaced for a plain turn) + `holon.test.mjs` (P127, with the passage-less control); `eoreader7/native/eval/the-fold/lib/long-stream.mjs` (`withoutAddresses` in `has`/`answerAtoms`) + `tests/long-stream.test.js`. Suites 1,662/0 and 654/0.

## P128 — The conversation's own record is retrievable: a question about what was said is answered from the record, not from the window (2026-09-05)

**Generality:** universal for the rules (a question that points back at what was said — "earlier I asked X, what did you answer", "you said Y" — retrieves the relevant prior turns from the transcript and hands them over as ADDRESSED passages (`turn:N`), exactly as a question about the material retrieves from the material, so every organ downstream reads them unchanged; the quoted question wins outright, otherwise content-word overlap decides, and a transcript with nothing in common hands over NOTHING rather than the most recent turns regardless — an absent history is a finding; a prior answer is evidence only of what was said, never of what is true). Local for the numbers (`RECALL_TURNS = 3`, the two-word overlap floor — declared, unmeasured).

**The measured gap.** The fold's standing claim is that the transcript grows without limit while the carried context does not, because the RECORD holds what the window drops. That was true for sources and false for the conversation itself: the turn passed only the last few messages of chat history, so a question about turn 5 asked at turn 50 reached the mouth with nothing to answer from. Measured (S77, clean corpus): the memory probe scored 0.00, 0.00, 0.25, 0.00.

**Two walls, both load-bearing.** A prior answer is NEVER admitted to the hyperlexicon as material — letting the mouth's own earlier words into the ledger would make the model its own witness (`self:model` may never corroborate itself, P2) and a claim would gain standing merely by being repeated. And it is kept out of the snips/obligations pool, which is about the material. The passages carry a line saying what they are: "this is what was said, which is not the same as what the sources establish."
...
## P129 — The pool coordinates which model runs where, each machine says what it is, and every turn declares which model on which machine answered each call (2026-09-06)

**Generality:** universal for the rules (a machine publishes what it serves, what it could serve, and what it measured itself to be — cores, memory, CPU or GPU, `null` where nothing has been measured; a member may ask a machine to take up a model it says it has spare, and the machine takes it up or refuses WITH a reason that travels; work is routed by measured expected wait, never by a count alone; a machine the person picked is pinned through every routing decision of a turn; and every call of a turn declares the model and the machine that answered it, stamped with the turn it belongs to); specimen-scoped for the models, hosts and timings measured on this machine.

**The asks.** "Test running the multiple machine inference." Then: "let it be able to coordinate which models, on CPU", and "make sure it declares what model did each task."

**What the drill found** (two headless workers as separate accounts with their own sessions, identities and homes, a scripted requester, and the browser as a fourth member, against the fake homeserver):
1. **Spreading worked, balancing did not.** Six concurrent asks went three and three by in-flight count — but one machine served a model six times slower, so its queue outlived the requester's patience: 4 of 6 answered, two typed timeouts. A worker answers one job at a time (one runtime, one GPU), so a queue is real waiting. `pickMouth` now ranks by expected wait — jobs in flight × how long that machine's answers have actually taken — with an unmeasured machine scored at what the measured ones take, so it is tried rather than starved. Re-run: 6 of 6, five to the fast machine.
2. **A job naming no model reached Ollama as a null** and came back as six typed gaps carrying "model is required". A worker now answers an unnamed job with the first model it offers, because "whatever this machine has" is a real ask and the worker is what knows.
3. **A deadline shorter than the queue reported "no answer" for a machine that was working.** The wait now stretches to the measured queue plus one more answer, and the gap says whether the machine is still offering (busy) or gone.
4. **The fixture only sent room state on the first sync**, so a worker never saw the room ask it for a model. Real homeservers send state deltas; the fixture now does too, and the serve loop re-reads state every 10s as a backstop.
5. **A picked machine was quietly overridden.** The routing ladder swaps in the fastest local rung for flat turns, and the two-pass turn forces its own specialists — so a turn taken under a room mouth ran entirely on the local model. `isPinnedModel` makes a `room:@who:server model` selection outrank both.
6. **Attribution crossed turns.** A fire-and-forget summary call from the previous turn landed under the next answer, and the self-citation cited a previous turn's model. Both are stamped with the turn now.
7. **The page pointed at a room its account could not reach** (a different sign-in on the same browser), and every door answered "not a member". It now forgets that room and says so.

**What shipped.** `matrix.js`: `mouthContent` carries `available`, `device` and `refused`; `deviceContent`/`deviceLine` (runtime, CPU or GPU, cores, memory, os/arch — `gpu: null` when unmeasured); `wantContent`/`wantsFor`; `pickMouth` by measured expected wait. `matrix-client.js`: `want`/`unwant`, `serve` takes up asked-for models it has spare (or refuses through `canTakeUp`), the ask's measured deadline and busy-or-gone gap, `pool()` reporting device, spare and refusals. `matrix-worker.mjs`: reads its own cores and memory, asks Ollama whether the work is on the GPU (`size_vram`), offers every other pulled model as spare, and takes up what the room asks for if it is pulled. `model-routing.js`: `isPinnedModel`. `app.js`: `/pool want @who <model>` and `/pool drop`, the pool table with machine and spare columns, the per-call attribution line under every answer, room mouths in `state.offeredModels`, the ground ladder's self-citation naming the mouth that actually spoke, and forgetting an unreachable room.

**Measured, 2026-09-06.** Coordination: a worker offering `gemma2:2b` listed 15 spare models; asked for `qwen2.5-coder:1.5b`, it took it up and answered with it in 5.9s; asked for a model nobody has, it refused with "this machine does not have that model", and the ask for it was refused at the door. The machine reported itself as `Ollama on localhost:11434 · GPU · 8 cores · 24 GB · darwin arm64`. Attribution, in the browser: a local turn read `2× gemma2:2b on this machine (10.4s)`; a turn under a room mouth read `2× qwen2.5-coder:1.5b on @workerb:fake.test's machine, through the room (8.6s)`, and the sentence's own citation named the same mouth. Suites: 39 Matrix tests, full the-fold 1720 pass, 0 fail.

**What is not claimed.** The two workers ran on one physical machine against one Ollama, so this measures the routing, the coordination and the accounting — not throughput across real hardware. A CPU-only machine was declared in tests, not run. Nothing schedules a model pull: a machine takes up only what it already has. The pool's picture of a machine is what that machine says about itself.

## P130 — Three parts, three grains: bytes keep their addresses, memory goes relative, and the meta part that relates them is written to the record (2026-09-07)

**Generality:** universal for the rules (a field of states reached only from a cue or one synapse along, with no key and no position, whose every verdict is relative to a null band measured on that field for that cue length; a ground address that is never rewritten, only re-anchored by a recorded act; a correspondence act that tells *moved* from *gone* and lands at grain Pattern resting on the address and the figure); specimen-scoped for the numbers measured on one text.

**The ask.** "Experiment with making it so all addresses are relative, so it is more like a network system running on the electromagnetic state of neurons than a graph database", then "consider if this sets up two parallel systems of address and therefore a third, a ground, figure, pattern on a whole system level", then "one part of the system has addresses, the other doesn't, and a third is meta somehow", then "write a spec to develop this fully."

**What was built to find out.** `relative.js`, a keyless field: a text lights bits (words and ordered pairs onto 4,096), nodes are states with synapses to what arrived before and after, `recall(cue)` settles activation and reports where; `after`/`before` walk one synapse; `serialize` names neighbours by signature so a shuffled store rebuilds the same field; there is no `get`. `relative-pattern.js`, the meta part: `drift` (exact, shifted, moved, gone), `reanchor` (a figure minted into a ground address, or a typed gap), `correspond` (agree, repaired, ground-only, ground-shifted, apart). `eval/relative-addresses.mjs` runs both against absolute addresses over one text with a null band per cue length. Seven tests.

**Measured, 2026-09-07** (`eval/results/relative-addresses-2026-09-07.txt`): addresses alone are exact or nothing — 120/120 on the whole passage and 0/120 under any fragment, corruption, shift or rename. The field alone recalls 119–120/120 from a 30% fragment or 40% corruption, 87/120 from 15% with 29 said *ambiguous*, and calls 0/120 random cues a figure. The pattern between them repairs 120/120 after a shift, a rename, or both, and in the deletion run its 24 *apart* verdicts are exactly the 24 deleted probes. A shuffled store rebuilt identically on all 120 probes. One recall over 400 nodes ≈ 1.4 ms.

**The reading, on the whole system.** The record and sources are the part with addresses (Ground). The model, and now this memory, are the part without (Figure). The checks — witnesses, null bands, the ground ladder, attribution, the leak instrument, and these correspondence acts — are the meta part (Pattern), which holds nothing of its own and is the only part that can tell moved from gone. In a brain all three are addressless, which is why a brain confabulates and cannot audit itself; here the meta part writes to the addressed part, which is why "it can be wrong but always shows its work" is keepable. So the rule is not "all addresses relative": the ends stay absolute, the middle goes relative, and what the middle decides is recorded. `GROUND-FIGURE-PATTERN-SPEC.md` develops that in Passes 32–39.

**What is not claimed.** Nothing is wired into the page; no reopen re-anchors yet; the field has not been run over the real record; the constants (4,096 bits, one spreading step at 0.25) are stated, not yet measured for sensitivity; the model's weights are untouched and stay so.

## P131 — Prediction as author is a reordering of the turn, not another program (2026-09-07)

**Generality:** universal for the rules (a turn's expected answer is composed from the record, the sources, the priors and the voids before the model drafts, and is on the record before the draft; the model renders it; the difference between the rendering and the expectation is the error, scored against a null, and the witnesses spend their budget on the error alone; the error is what updates the record and the learned priors; the self tier stays open as the channel novelty enters through; precision is the tier and never a chosen weight); specimen-scoped for nothing yet — no pass has run.

**The ask.** After the assessment that likened the system to a brain, the spec of the same day put prediction-as-author out of scope as "a separate program". The user: "why can't prediction be an author in our system? we have all the Bayesian mechanics."
