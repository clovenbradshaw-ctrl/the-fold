...
**How to use this.** §3 is nineteen small tasks, in order. Do them in order and
...
## 3. The nineteen tasks

Nineteen tasks in four phases. Each says what to do, how you know it's finished,
...
points send you backwards, and each sits inside the next: Task 13 sends you
back to Task 10 (fan out from a new source), Task 14 can send you back to Task 5
(redraw the gap), and Task 19 can send you back to Task 1 (the ask itself was
...
### Phase A: describe the gap (Tasks 1–8)
...
> **Task 4: Get to the ground.**
> **A ground is not a baseline you look up. It is a nothing you construct**, out
> of what is already in front of you, and for a product what is already in front
> of you is the product. So this task is not research. It is saturation: you
> learn the thing you already shipped until it stops registering. Here: *the
...
> is most of the product. If nobody can, you are not ready to describe a gap in
> it. The section after this one is how you get there, and how you tell.
> **Trap:** skipping it, because it doesn't feel like progress. Nothing gets
> proposed in this task and nothing gets decided, which makes it the easiest
> thing in the method to drop and the most expensive thing to have dropped: a
> room with no ground can count what it covers and can never tell whether
> covering it did anything at all.

> **Task 5: Draw the extent, and give it units.**
...
> which there are six.
> **Not the same thing as Task 4, and this is the pair people collapse.** The
> **ground** is what everything has to stand out *from*. The **extent** is what
> has to be covered, cut into countable pieces: it tells you *where to look*.
> Only one of the two is arithmetic. (The **gap** is the whole description
> you're building across Tasks 1–8, both of these included.)
> **Done when:** you can write the pieces out as a list: an actual list, with
> lines on it. If it won't come, try a different angle before you decide the
> units are wrong. See below.
...
there is nothing honest to say about it yet. This is what Task 11 is already
reaching for when it asks for **a witness** rather than an opinion, and Task 4
...
**The trap, and it has the same shape as Task 7's.** Deciding what counts as
...
### If Task 5 won't come: six angles to try on the same gap
...
> was really two, and the units you wrote in Task 5 were too coarse. That is
...
> **Task 7: Write the test, before anyone has a favourite.**
...
**And it's the test of whether you actually did Task 5.** If you can't write this
...
### Phase B: fill it (Tasks 9–13)

> **Task 9: Name the fake subject, and park it.**
...
> **Task 10: Fan out, in silence, from one source only.**
> Everyone writes proposals for the gap from Task 2–5. Start with the source
...
> **Task 11: Judge them one at a time, against Task 7.**
...
> **Task 12: Mark the coverage and name the hole.**
> Put what passed onto the extent from Task 5. Count it. Then **name what's left
...
> **Task 13: If it's still short, change source and go again.**
...
### Phase C: check the gap itself (Tasks 14–16)

> **Task 14: Look for the three signs you drew the gap wrong.**
...
> **Task 15: If a sign fired, redraw the gap.**
...
> **Task 16: Write the questions that would settle it, in order.**
...
### Phase D: commit, and leave a way back (Tasks 17–19)

> **Task 17: Give each survivor a standing.**
...
> **Task 18: Check the three gates before calling anything committed.**
...
Keep them apart. "Can't tell" isn't a failure: it's a question, and Task 16 tells
...
**To become a candidate:** the gap is described (Tasks 1–8), the kill line exists,
...
**To become a commitment:** all three gates from Task 18.
...
**It cannot be judged, so it can never be refused.** Task 11 gives every proposal
...
**It falsifies the count.** Put it on the extent from Task 5 and it appears to
...
That's Task 9, and it comes first in Phase B for a reason: a fake subject can
...
Task 8 is for: it's the only part of this that reaches outside the room.
...
running. Watch for Task 13 (the innermost loop, same gap, new source) and
Tasks 14–15 (the middle loop, the gap itself gets redrawn). Neither is a
...
**Tasks 1–8.** The ask, written down: Priya's photo ID and Marcus's sound ID,
...
**Checkpoint.** The list goes on the wall: six lines, two ticked, four not, and the four unticked ones get read out loud, with Priya and Marcus both in
...
**Task 9.** "The ID engine" named and parked, in writing.

**Task 10.** Silent fan-out from **source 1, what people already told you**:
...
**Task 11.** Photo ID **passes** (what they saw, runs on the phone). Sound ID
...
**Task 12.** Two of six. Uncovered: where they were, what time of year, what
the bird was doing, how long they had.

**Task 13, and this is the innermost loop firing.** Source 1 is spent:
...
Task 10 with the same gap and a different well.

**Tasks 10, 11 and 12 again, on source 2.** Two more proposals: a "likely here,
...
**Task 14, and this is the good part.** Someone writes in about the sighting
...
**Task 15.** Redraw: split "what they saw" into *seen well* and *seen poorly*.
...
**Task 16**, written last, because the redraw changed what's worth asking.
...
**Tasks 17–19.** Sound ID is a **candidate**, and so is photo ID, but "bird
...
neither was built to notice. Second, and this is Task 11's catch coming due:
...
**Two of the three loops fired inside a single session.** L1 at Task 13, when
source 1 ran out at two of six. L2 at Task 14, when the 400-metre bird wouldn't
...
Task 19 is what it will look like when it does.
...
- **The ground (4):** the booking app as it already is. Decision named first,
  *what do we do between booking and appointment*. Then: drop the staff calendar,
  same answer; drop the intake form, the room allocation, the clinician's own
  view, same answer. All background. Drop *a reminder already goes out the day
  before* and the answer moves, so that one is live. What won't recede: a patient
  who meant to come, and nothing in that gap reached them.
- **The extent (5):** the time between booking and appointment. **Units: hours
  before the appointment.**
- **Fitting (6):** contacts stack: a reminder and a confirmation can both happen.
  Changes to the appointment itself exclude each other.
- **The test (7):** it must say which hours-before window it acts in, and the
  clinic must already be able to do it.
- **The kill line (8):** don't do it for a random half: if missed appointments
  fall the same either way, this whole picture is wrong.

**Fan-out (10):** a text at 24 hours, a text at 2 hours, one-tap confirm, one-tap
reschedule, filling the slot from a waiting list.

**Judging (11):** the waiting list **fails**: it doesn't change whether *this*
...
**Then two signs fire (14).** *Throwing away answers unread:* help with transport,
...
Both are Task 15: redraw wider and finer, keep what passed, put back what the old
boundary excluded.
...
|                                                                     |
|   +-- L2 - redraw the gap ---------------- re-enters at task 5 --+  |
|   |   A sign fired at task 14. Keeps what already passed,        |  |
|   |   and restarts every L1 inside it.                           |  |
|   |                                                              |  |
|   |   +-- L1 - change source --------- re-enters at task 10 --+  |  |
...
*Fires at task 13, re-enters at task 10.*
...
*Fires at task 14, re-enters at task 5.*
Fires when: one of the three signs in Task 14 goes off: something's aimed at
...
Redrawn, two of the six evidence kinds from Task 5 turned out not to matter at
that narrower moment, and a proposal parked back in Task 11, "ask the
...
*Fires after task 19, re-enters at task 1.*
...
shifts anyway. Here, from Task 19: a bird's range moves, and "likely here,
...
true. It's why Task 19 makes you write the give-up condition *before* you need
...
rewrites the gap but keeps every proposal that already passed, Task 15 says
...
people in it.** That isn't bureaucracy: it's gate (b) of Task 18, and it's the
...
- **It won't tell you if you're right.** Everything here checks whether you're
  ...
  completely wrong about the world. Only Task 8's kill line reaches outside the
...
**Describe the gap first (Tasks 1–8).** The ask, word for word · the moment (one
person, one occasion) · what's out · **the ground** (the product you already
have, known until it recedes) · **the extent, in countable units** · how pieces
fit · the test, written before anyone has a favourite · and the kill line.

**What you're aiming for:** a list of the parts, ticked where you cover them, and you can read the unticked ones out loud, by name. If you can't write the list,
that's the session's finding.

**Fill it (9–13).** Park the fake subject. Fan out in silence from what people
...
**Three signs you drew the gap wrong (14):** it's aimed at the wrong people · it's
rejecting answers unread while the problem stays open · it looks full but
something won't fit.

**Three questions, in order (16):** place what you already have → fill the hole →
re-ask what came back blank.

**Three standings (17):** hunch → candidate → commitment. And a route back (19): a commitment you can't undo isn't a commitment.
