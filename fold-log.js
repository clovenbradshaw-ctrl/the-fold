// fold-log.js — anything folded is an append-only log with EO notation.
//
...
// scrubbed to, downloadable there. "The code" is not a field that gets
// overwritten; it is the answer to "project the log up to here."
//
...
//               fold thread; the conformance test pins this against the
...
// the log and every cursor position still projects back byte-identically.
//
// This module is the disclosed amendment to "this repo is not a cube
// consumer" (CLAUDE.md, holonic layer): for FOLDS, and folds only, the
...
//
// Renamed from build-log.js (2026-08-17, user direction): "these are called
// folds, not builds" — the chat's own UI had already said "fold N" and
// "/fold <n>" while every identifier and file underneath it said "build".
// The one internal collision this rename had to resolve: `foldBuild(log)`
// meant "project the log to its current state", using "fold" as the VERB
// this whole instrument already uses everywhere (foldPace, advanceSummaryFold,
// the conversation's own running summary). Renaming the NOUN "build" to
// "fold" would have produced `foldFold` — so the noun becomes "fold" and the
// verb becomes `projectFold`, keeping both senses distinct rather than
// colliding them.

/**
 * Bind the fold-log operations to the engine's task-log module.
 *
 ...
 */
export function makeFoldLog(taskLog) {
  ...
  // Ground 1 keeps the plain `f<n>.v<k>` address it has always had — a
  ...
  const vid = (n, k, ground = 1) => (ground > 1 ? `f${n}.g${ground}.v${k}` : `f${n}.v${k}`);
  const rezeroId = (n, ground) => `f${n}.rezero.${ground}`;

  /** Project the log up to (and including) `atSeq` — the fold as of that
   * point. Omit `atSeq` for the live fold. `null` means the thread has no
   * live version there (nothing yet, or retracted). */
  function projectFold(log, atSeq = Infinity) {
    ...
    // evidence about the fold, never superseded), and they carry no code.
    // Projecting to one would blank the fold at exactly the cursor position
    // where the reader most wants to see what was conceded.
    ...
    // is the fold as it now stands. projectTasks sorts by first_seq, so
    // "last" is "most recently born", not an accident of Map order.
    const t = tasks[tasks.length - 1];
    // The instruction is a fold-log field, not engine vocabulary —
    // projectTasks does not carry it. Read it from the PROPOSE entry
    // that birthed this thread (the one matching this fold's n).
  ...
  /** A fold is born: the artifact the turn produced, snipped and named.
   ...
   *  travels through the projection. */
  function proposeFold({ n, turn, seg, caption, instruction = null }) {
   ...
   */
  function rezeroFold(log, { code, seg = null, caption = null, trigger, tell = null } = {}) {
    const cur = projectFold(log);
    if (!cur) return log;
    if (typeof trigger !== "string" || !trigger.trim())
      throw new TypeError("rezeroFold: a re-zero records the operator's own words as its trigger");
    const ground = groundCount(log) + 1;
    const nextSeg = seg ?? cur.seg;
    const nextCode = code ?? (nextSeg?.type === "code" ? nextSeg.code : null);
    // Churn is refused here exactly as it is in reviseFold: an identical
   ...
   */
  function reviseFold(log, { code, reason = "edit" } = {}) {
    const cur = projectFold(log);
  ...
  /** The fold leaves the live set. Its entries remain — a retraction is an
   * entry too, never an erasure. */
  function retractFold(log) {
    const cur = projectFold(log);
    if (!cur) return log;
  ...
  /**
   * The fold as a downloadable file, at any cursor position. The seq is in
   * the name because it IS the address: `fold-3@5.py` says exactly which
   * projection of the log these bytes are. `toDocument` is injected
   * (artifact.js owns it) so an html fold downloads as the same complete
   * document the preview renders.
   */
  function exportAt(log, atSeq, { toDocument = null } = {}) {
    const b = projectFold(log, atSeq ?? Infinity);
    if (!b) return null;
    ...
    if (lang === "html") {
      return {
        name: `fold-${b.n}@${seq}.html`,
        mime: "text/html",
        text: toDocument ? toDocument({ ...seg, code }) : code,
      };
    }
    if (lang === "svg") {
      return { name: `fold-${b.n}@${seq}.svg`, mime: "image/svg+xml", text: code };
    }
    return {
      name: `fold-${b.n}@${seq}.${EXT[lang] ?? "txt"}`,
      mime: "text/plain",
      text: code,
    };
  }
  ...
  /**
   * Migrate a pre-log fold ({seg, code, lastRun} — the mutable shape this
  ...
  function fromLegacy({ n, turn, seg, caption, code, lastRun }) {
    let log = proposeFold({ n, turn, seg, caption });
    if (seg?.type === "code" && typeof code === "string" && code !== seg.code) {
      log = reviseFold(log, { code, reason: "edit" });
    }
