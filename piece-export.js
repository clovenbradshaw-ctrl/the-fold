...
export function exportPiece({ title = "Untitled", ask = "", model = null, sections = [], passages = new Map(), urls = {}, prompts = {}, stats = null, generatedAt = null } = {}) {
  ...
  const script = `(function(){var panel=document.getElementById('snip');var body=document.getElementById('snip-body');var title=document.getElementById('snip-title');function esc(t){return String(t).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}function show(el){var spans=[];try{spans=JSON.parse(el.getAttribute('data-spans')||'[]')}catch(e){spans=[]}var tier=el.getAttribute('data-tier')||'';title.textContent=(el.getAttribute('data-phrase')||tier)+(el.getAttribute('data-detail')?' — '+el.getAttribute('data-detail'):'');if(!spans.length){body.innerHTML='<p class="none">'+esc(tier==='self'?'Nothing read places this sentence; it is the model\u2019s own testimony.':'No cited span for this sentence.')+'</p><p class="sent-copy">'+esc(el.firstChild&&el.firstChild.nodeType===3?el.firstChild.textContent:el.textContent)+'</p>';panel.hidden=false;return}var html='';spans.forEach(function(sp){var t=document.getElementById(sp.p);var text=t?t.textContent:'';var a=sp.start,b=sp.end;if(a==null||b==null){var i=text.indexOf(sp.text);if(i>=0){a=i;b=i+sp.text.length}}var marked=(a!=null&&b!=null&&a>=0&&b<=text.length)?esc(text.slice(0,a))+'<mark>'+esc(text.slice(a,b))+'</mark>'+esc(text.slice(b)):esc(text);html+='<div class="src"><div class="src-head"><code>'+esc(sp.ref)+'</code>'+(sp.link?' <a href="'+esc(sp.link)+'" target="_blank" rel="noopener">open the page at the span</a>':'')+'</div><pre class="src-text">'+marked+'</pre></div>'});body.innerHTML=html;panel.hidden=false;var m=body.querySelector('mark');if(m)m.scrollIntoView({block:'center'})}document.querySelectorAll('.s').forEach(function(el){el.addEventListener('click',function(e){if(e.target.tagName==='A')return;show(el)});el.addEventListener('keydown',function(e){if(e.key==='Enter')show(el)})});document.getElementById('snip-close').addEventListener('click',function(){panel.hidden=true})})();`;
