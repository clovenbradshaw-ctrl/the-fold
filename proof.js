...
// An ACRONYM is content, however short it is. The length floor below exists
// to drop noise, and it silently dropped the only word that said what a
// question was ASKING — measured live 2026-08-26 on the real app, the whole
// chain visible end to end: "who was lincoln's VP?" built the search query
// `"lincoln"` (VP is two characters), DuckDuckGo answered with eight Lincoln
// Motor Company pages, the preflight fetched three, and the model was handed
// luxury-SUV marketing copy to answer a question about a vice president. The
// instrument caught its own failure correctly downstream ("not supported by
// that material") — but the defect was upstream of every check, in what was
// searched for. Two worse cases from the same floor: "what did the US do"
// reduced to the EMPTY string (nothing survives), and "explain AI safety"
// searched "explain safety".
//
  ...
  const content = (s) => [
    ...new Set(
      String(s ?? "")
        .split(/[^\p{L}\p{N}'’]+/u)
        .map((w) => w.replace(/['’]s$/, ""))
        .filter((w) => ACRONYM.test(w) || (w.length > 2 && !CLAIM_STOPWORDS.has(w.toLowerCase()))),
    ),
  ];
