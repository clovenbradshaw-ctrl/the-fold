...
test("LENS: notes, voids and checked turns whose ends resolve to the active referents — grouped by referent, standing phrased from the ledger's own table, a dispute named, addresses carried", () => {
  ...
  assert.match(l.text, /Porfiry Petrovich — questioned→ Raskolnikov \[pg2554\.txt#48673-52190, pg2554\.txt#90000-91000\] \(stated once so far\)/);
  ...
  assert.match(l.text, /earlier in this conversation \[turn:4\]: Porfiry Petrovich questioned Raskolnikov twice\./);
  assert.doesNotMatch(l.text, /Razumihin — brought/, "a note about another referent is not shown");
  assert.match(l.text, /earlier in this conversation \[turn:5\]: Each time Porfiry smiled\./, "two checked turns citing two addresses are two identities");
  ...
  assert.match(r.text, /Rodion Raskolnikov — murdered→ the old woman \[pg2554\.txt#45324-48671, other\.txt#10-20\] \(read in 2 places; disputed by other\.txt — not settled\)/, "a note under dispute says so, and 'Rodion' resolves to the same referent");
  clean(r.text);
