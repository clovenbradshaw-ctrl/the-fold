 ...
 * off the model record's own declarations — `required_features` (an adapter
 * that lacks `shader-f16` is exactly the offered-then-fails-on-load trap this
 * closes, 2026-09-11) and `buffer_size_required_bytes` against the adapter's
 * `maxStorageBufferBindingSize` — and the WebGPU adapter's own report
 * (injected; the page probes `navigator.gpu.requestAdapter()`).
 *
 * `features` null means NO usable adapter (requestAdapter returned none —
...
export function rungBlockers(entry, adapter = {}) {
  ...
  const out = [];
  if (features == null) {
    out.push(`no usable WebGPU adapter on this device, and ${entry.model_id} needs one to run`);
    return out;
  }
  const have = features instanceof Set ? features : new Set(features);
  for (const f of entry?.required_features ?? []) if (!have.has(f)) out.push(`this adapter lacks ${f}, which ${entry.model_id} requires`);
